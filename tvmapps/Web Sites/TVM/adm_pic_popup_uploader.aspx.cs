        ...
        if (id > 0)
        {
            switch (picMediaType)
            {
                ...
                case PopUpContext.EpgProgram:
                    {
                        picId = TvinciImporter.ImporterImpl.DownloadEPGPicToImageServer(picLink, name, groupID, id, ratioId, 0, false,LoginManager.GetLoginID(),Session["epgIdentifier"].ToString());
