                        ...
                        && Session["parent_limit_id"].ToString().Length > 0)
                    {
                        ...
                        if (limitID > 0 && updatedDeviceFamilyIDs != null && groupID > 0)
                        {
                            ...
                            try
                            {
                                ...
                                TVM.DomainsWS.module p = new TVM.DomainsWS.module();
                                string sIP = "1.1.1.1";
                                string sWSUserName = "";
                                string sWSPass = "";
                                ...
                                try
                                {
                                    TVM.DomainsWS.Status resp = p.RemoveDLM(sWSUserName, sWSPass, parentLimitID);
    ...
    public string changeItemStatus(string sID, string sAction)
    {
        ...
        if (Session["device_families"] != null && Session["device_families"] is List<UMObj>)
        {
            ...
            if (Session["parent_limit_id"] != null && Session["parent_limit_id"].ToString().Length > 0)
            {
                ...
                try
                {
                    TVM.DomainsWS.Status resp = p.RemoveDLM(sWSUserName, sWSPass, limitID);
