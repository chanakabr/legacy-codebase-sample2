                        ...
                        if (epgCB != null)
                        {
                            ...
                            if (m_bConfirm) // confirm button
                            {
                                if (epgCB.Status == 4) //remove permanent
                                {
                                    ...
                                    bool result = false;
                                    result = ImporterImpl.UpdateEpg(new List<ulong>() { epgCB.EpgID }, nGroupID, ApiObjects.eAction.Delete, null, false);
                                }
                                else if (epgCB.Status == 1)
                                {
                                    ...
                                    bool result = false;
                                    result = ImporterImpl.UpdateEpg(new List<ulong>() { epgCB.EpgID }, nGroupID, ApiObjects.eAction.Update, null, false);
                                }
                            }
                            else // cancel button
                            {
            ...
            switch (m_sTable.ToLower())
            {
                ...
                case "groups_device_families_limitation_modules":
                    ...
                    if (oDlmID != null)
                    {
                        ...
                        try
                        {
                            TVM.DomainsWS.Status resp = domainWS.RemoveDLM(sWSUserName, sWSPass, dlmID);
                ...
                case "groups_device_limitation_modules":
                    ...
                    try
                    {
                        TVM.DomainsWS.Status resp = domainWS.RemoveDLM(sWSUserName, sWSPass, m_nID);
                ...
                case "domains":
                    ...
                    try
                    {
                        ...
                        TVM.DomainsWS.DomainResponseStatus resp = domainWS.RemoveDomain(sWSUserName, sWSPass, m_nID);
                ...
                case "payment_gateway_config":
                    ...
                    if (oPaymentGatewayId != null && oPaymentGatewayId != DBNull.Value)
                    {
                        ...
                        try
                        {
                            TVM.Billing.Status status = billing.SetPaymentGatewayConfiguration(sWSUserName, sWSPass, paymentGatewayId);
