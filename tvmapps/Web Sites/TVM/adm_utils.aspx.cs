    ...
    public string ChangeVid(string sObjectID, string sPicsIds, string sTable)
    {
        ...
        for (int i = 0; i < sSplit.Length; i++)
        {
            if (sSplit[i] != null && sSplit[i].ToString() != "")
            {
                ...
                string sPicURL = ImageUtils.GetImageUrl(picId, groupId);
        ...
        string sRequestedStatus, string sOnStr, string sOffStr, string sConnKey)
    {
        ...
        if (sTableName == "Epg")
        {
            ...
            if (epgCB != null)
            {
                ...
                bool result = false;
                result = ImporterImpl.UpdateEpg(new List<ulong>() { epgCB.EpgID }, nParentGroupID, ApiObjects.eAction.Update, null, false);
