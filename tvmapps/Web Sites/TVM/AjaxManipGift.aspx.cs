...
public partial class AjaxManipGift : System.Web.UI.Page
{
    protected void AddUserLog(string sSiteGUID, string sRemarks)
    {
        ...
        TVM.users_ws.UsersService p = new TVM.users_ws.UsersService();
        string sIP = "1.1.1.1";
        string sWSUserName = "";
        string sWSPass = "";
    ...
    protected TVM.ca_ws.BillingResponseStatus GiveFreeSubOld(string sSiteGUID, string sGiftCode, string sRemarks, ref string sError)
    {
        Int32 nGroupID = TVinciShared.LoginManager.GetLoginGroupID();
        TVM.ca_ws.module p = new TVM.ca_ws.module();
        string sIP = "1.1.1.1";
        string sWSUserName = "";
        string sWSPass = "";
    ...
    protected TVM.ca_ws.BillingResponseStatus GiveFreeSub(string sSiteGUID, string sGiftCode, string sRemarks, ref string sError)
    {
        TVM.ca_ws.BillingResponseStatus ret = TVM.ca_ws.BillingResponseStatus.Fail;
        ...
        var res = p.GrantEntitlements(sWSUserName, sWSPass, sSiteGUID, 0, 0, int.Parse(sGiftCode), TVM.ca_ws.eTransactionType.Subscription, "1.1.1.1", string.Empty, true);
        sError = res.Message;
        ret = res.Code == 0 ? TVM.ca_ws.BillingResponseStatus.Success : TVM.ca_ws.BillingResponseStatus.Fail;

        return ret;
    }

    protected TVM.ca_ws.BillingResponseStatus GiveFreePPVOld(string sSiteGUID, string sGiftCode, string sRemarks, ref string sError)
    {
        TVM.ca_ws.BillingResponseStatus ret = TVM.ca_ws.BillingResponseStatus.Fail;
        ...
        if (int.TryParse(sGiftCode, out mediaFileId))
        {
            ...
            if (selectQuery.Execute("query", true) != null)
            {
                ...
                if (count > 0)
                {
                    ...
                    TVM.ca_ws.BillingResponse response = p.CC_DummyChargeUserForMediaFile(sWSUserName, sWSPass, sSiteGUID, 0, "USD", int.Parse(sGiftCode), ppvModuleID, "", "1.1.1.1", "", "", "", "");
    ...
    protected TVM.ca_ws.BillingResponseStatus GiveFreePPV(string sSiteGUID, string sGiftCode, string sRemarks, ref string sError)
    {
        TVM.ca_ws.BillingResponseStatus ret = TVM.ca_ws.BillingResponseStatus.Fail;
        ...
        if (int.TryParse(sGiftCode, out mediaFileId))
        {
            ...
            if (productId > 0)
            {
                var res = p.GrantEntitlements(sWSUserName, sWSPass, sSiteGUID, 0, mediaFileId, productId, TVM.ca_ws.eTransactionType.PPV, "1.1.1.1", string.Empty, true);
                sError = res.Message;
                ret = res.Code == 0 ? TVM.ca_ws.BillingResponseStatus.Success : TVM.ca_ws.BillingResponseStatus.Fail;
            }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (sType == "sub")
        {
            TVM.ca_ws.BillingResponseStatus ret = GiveFreeSub(sSiteGUID, sGiftCode, sRemarks, ref sError);
            if (ret == TVM.ca_ws.BillingResponseStatus.Success)
                bOK = true;
        }
        if (sType == "ppv")
        {
            TVM.ca_ws.BillingResponseStatus ret =  GiveFreePPV(sSiteGUID, sGiftCode, sRemarks, ref sError);
            if (ret == TVM.ca_ws.BillingResponseStatus.Success)
                bOK = true;
        }
        if (bOK == true)
