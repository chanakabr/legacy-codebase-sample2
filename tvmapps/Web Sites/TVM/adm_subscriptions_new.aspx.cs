...
public partial class adm_subscriptions_new : System.Web.UI.Page
{
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        if (int.Parse(Session["lang_id"].ToString()) == GetMainLang(ref sMainLang, ref sMainCode))
        {
            ...
            TVM.TvinciPricing.PriceCode[] oModules = m.GetPriceCodeList(sWSUserName, sWSPass, string.Empty, string.Empty, string.Empty);
            if (oModules != null)
            {
                for (int i = 0; i < oModules.Length; i++)
                {
                    ...
                    /*
                    TVM.TvinciPricing.LanguageContainer[] lang = oModules[i].m_sDescription;
                    if (lang != null)
                    {
            ...
            TVM.TvinciPricing.UsageModule[] oUMCodes = m.GetUsageModuleList(sWSUserName, sWSPass, string.Empty, string.Empty, string.Empty);
            if (oUMCodes != null)
            {
            ...
            TVM.TvinciPricing.DiscountModule[] oDiscCodesExternal = m.GetDiscountsModuleListForAdmin(sWSUserName, sWSPass);
            if (oDiscCodesExternal != null)
            {
                for (int i = 0; i < oDiscCodesExternal.Length; i++)
                {
                    ...
                    /*
                    TVM.TvinciPricing.LanguageContainer[] lang = oDiscCodes[i].m_sDescription;
                    if (lang != null)
                    {
                        string sMainLang = GetMainLang();
                        for (int j = 0; j < lang.Length; j++)
            ...
            TVinciShared.WS_Utils.GetWSUNPass(LoginManager.GetLoginGroupID(), "GetDiscountsModuleListForAdmin", "pricing", sIP, ref sWSUserName, ref sWSPass);
            TVM.TvinciPricing.DiscountModule[] oDiscCodes = m.GetDiscountsModuleListForAdmin(sWSUserName, sWSPass);
            if (oDiscCodes != null)
            {
            ...
            TVM.TvinciPricing.CouponsGroup[] oCouponsGroup = m.GetCouponGroupListForAdmin(sWSUserName, sWSPass);
            if (oCouponsGroup != null)
            {
                for (int i = 0; i < oCouponsGroup.Length; i++)
                {
                    ...
                    /*
                    TVM.TvinciPricing.LanguageContainer[] lang = oCouponsGroup[i].m_sDescription;
                    if (lang != null)
                    {
                        string sMainLang = GetMainLang();
                        for (int j = 0; j < lang.Length; j++)
