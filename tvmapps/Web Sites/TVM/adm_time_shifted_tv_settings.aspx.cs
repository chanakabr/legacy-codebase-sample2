...
public partial class adm_time_shifted_tv_settings : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["submited"] != null && Request.QueryString["submited"].ToString() == "1")
            {
                ...
                TVM.apiWS.TimeShiftedTvPartnerSettings tstvOld = GetTimeShiftedTVSettings(groupId);

                int id = DBManipulator.DoTheWork("MAIN_CONNECTION_STRING");
                if (id > 0)
                {
                    TVM.apiWS.TimeShiftedTvPartnerSettings tstvNew = GetTimeShiftedTVSettings(groupId);
    ...
    private TVM.apiWS.TimeShiftedTvPartnerSettings GetTimeShiftedTVSettings(int groupId)
    {
        TVM.apiWS.TimeShiftedTvPartnerSettings tstv = null;
        ...
        if (selectQuery.Execute("query", true) != null)
        {
            ...
            if (nCount > 0)
            {
                tstv = new TVM.apiWS.TimeShiftedTvPartnerSettings()
                {
