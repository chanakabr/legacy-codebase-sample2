<%@ Page Language="C#" AutoEventWireup="true" Inherits="admin_category_chooser" Codebehind="admin_category_chooser.aspx.cs" %>
...
<head id="Head1" runat="server">
    ...
    var itemRequieredType = "<% GetRequieredType(); %>";
    function initPage(){
    ...
    function selectTreeItem(flashID, itemType, itemID){
        //alert(flashID);
        //alert(itemType); //channel //category
        if (itemRequieredType != itemType)
        {
            alert("You cannot choose '" + itemType + "' item. Please choose '" + itemRequieredType + "' item.");
        }else
        {
	        parent.document.getElementsByName("<% GetElementID(); %>")[0].value = itemID;
	        var elementName = 'TVMItemView_' + '<% GetElementNum(); %>';
	        parent.document.getElementsByName( elementName )[0].innerHTML = "(Current : " + itemID + ")";	        
	    }
    }
    
    function changeTvmAccount(playerUN , playerPass)
    {
        alert(playerUN + "|" + playerPass);
    }
