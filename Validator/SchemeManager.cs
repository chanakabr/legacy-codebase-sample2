...
namespace Validator.Managers.Scheme
{
    public class SchemeManager
    {
        ...
        internal static bool Validate(MethodInfo action, bool strict)
        {
            ...
            {
                if (!hasValidationException(action, SchemeValidationType.ACTION_RETURN_TYPE) && action.ReturnType.Name.ToLower() != expectedObjectType.ToLower())
                {
            ...
            if (!hasValidationException(action, SchemeValidationType.ACTION_ARGUMENTS))
            {
                ...
                if (actionId == "add")
                {
                    ...
                    else
                    {
                        ...
                        if (!hasValidationException(action, SchemeValidationType.ACTION_RETURN_TYPE) && objectParam.ParameterType.Name.ToLower() != expectedObjectType.ToLower())
                        {
            ...
            if (actionId == "list" && !hasValidationException(action, SchemeValidationType.ACTION_RETURN_TYPE))
            {
