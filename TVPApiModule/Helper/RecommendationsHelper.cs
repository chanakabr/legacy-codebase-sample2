...
namespace TVPApiModule.Helper
{
    ...
    public class RecommendationsHelper
    {
        ...
        internal static dsItemInfo GetVodRecommendedMediasFromCatalog(int groupID, PlatformType platform, string picSize, List<VideoRecommendation> videoRecommendations)
        {
            ...
            APISearchMediaLoader loader = new APISearchMediaLoader(groupID, platform, string.Empty, SiteHelper.GetClientIP(), string.Empty, 0, 0, picSize, string.Empty)
            {
                //Tags = tags,
        ...
        {
            string retVal = null;
            dsItemInfo media = new TVPApiModule.CatalogLoaders.APIMediaLoader(mediaID, groupID, platform, string.Empty, SiteHelper.GetClientIP(), string.Empty, string.Empty).Execute() as dsItemInfo;
        ...
        internal static List<Media> GetFailOverChannel(TVPApiModule.Objects.InitializationObject initObj, int groupID, int channelID, string picSize, int maxResults)
        {
            List<Media> retVal = null;

            // get channel medias 
            List<MediaObj> medias = new APIChannelMediaLoader(channelID, groupID, initObj.Platform, initObj.UDID, SiteHelper.GetClientIP(), maxResults, 0, picSize, initObj.Locale.LocaleLanguage, null, null, CutWith.OR).Execute() as List<MediaObj>;
            if (medias != null && medias.Count > 0)
            {
                retVal = new List<Media>();
                foreach (MediaObj row in medias)
                    (retVal as List<Media>).Add(new Media(row, picSize, long.Parse(medias.Count.ToString()), groupID, initObj.Platform));
            }
            return retVal;
