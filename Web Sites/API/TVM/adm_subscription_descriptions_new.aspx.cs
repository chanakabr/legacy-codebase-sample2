...
public partial class adm_subscription_descriptions_new : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["subscription_description_id"] != null &&
                Request.QueryString["subscription_description_id"].ToString() != "")
            {
                Session["subscription_description_id"] = int.Parse(Request.QueryString["subscription_description_id"].ToString());
                Int32 nOwnerGroupID = int.Parse(ODBCWrapper.Utils.GetTableSingleVal("subscription_descriptions", "group_id", int.Parse(Session["subscription_description_id"].ToString()), "pricing_connection").ToString());
                ...
                Session["subscription_description_id"] = 0;
        }
    ...
    public void GetHeader()
    {
        ...
        string sRet = PageUtils.GetPreHeader() + ": Subscription descriptions (" + Session["subscription_id"].ToString() + ")";
        if (Session["subscription_description_id"] != null && Session["subscription_description_id"].ToString() != "" && Session["subscription_description_id"].ToString() != "0")
            sRet += " - Edit";
        else
            sRet += " - New";
        Response.Write(sRet);
    ...
    {
        string sBack = "adm_subscription_descriptions.aspx?search_save=1&subscription_id=" + Session["subscription_id"].ToString();
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        object t = null; ;
        if (Session["subscription_description_id"] != null && Session["subscription_description_id"].ToString() != "" && int.Parse(Session["subscription_description_id"].ToString()) != 0)
            t = Session["subscription_description_id"];
        string sBack = "adm_subscription_descriptions.aspx?search_save=1&subscription_id=" + Session["subscription_id"].ToString();
        DBRecordWebEditor theRecord = new DBRecordWebEditor("subscription_descriptions", "adm_table_pager", sBack, "", "ID", t, sBack, "");
        ...
        string sTable = theRecord.GetTableHTML("adm_subscription_descriptions_new.aspx?submited=1");
