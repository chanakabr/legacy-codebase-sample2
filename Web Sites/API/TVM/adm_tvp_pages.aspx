<%@ Page Language="C#" AutoEventWireup="true" CodeFile="adm_tvp_pages.aspx.cs" Inherits="adm_tvp_pages" %>
...
<script type="text/javascript">
    ...
    {
        sURL = "AjaxPageAddress.aspx?page_id=" + escape(val);
        postFile(sURL , callback_copyToCB);
    }
    function openPage(val , platform)
    {
        sURL = "AjaxPageAddress.aspx?page_id=" + escape(val) + "&platform=" + platform;
        postFile(sURL , callback_PageAddress);
    }
    function callback_PageAddress() {
            ...
            if (xmlhttp.status == 200) 
            {
                ...
                if (result == "WRONG_USERNAME_PASS") {
                    ...
                    OpenID("error");
                     ...
                     moveToBlank(result1.split("~~|~~")[1]);
                }
            }
        }
    }
    function callback_copyToCB() {
        if (xmlhttp.readyState == 4) {
            if (xmlhttp.status == 200) 
            {
                result1 = xmlhttp.responseText;
                ...
                if (result == "WRONG_USERNAME_PASS") {
                    ...
                    copyToClipboard(result1.split("~~|~~")[1]);
                    alert("Done!");
                }
            }
        }
    ...
    function GetPageTable(orderBy , pageNum)
    {
        ...
        RS.Execute("adm_tvp_pages.aspx", "GetPageContent", orderBy, pageNum, search_free, search_on_off, callback_page_content, errorCallback);
    }
    function create_csv()
    {
        RS.Execute("adm_tvp_pages.aspx", "GetTableCSV", callback_create_csv, errorCallback);
    }
</script>
</head>
