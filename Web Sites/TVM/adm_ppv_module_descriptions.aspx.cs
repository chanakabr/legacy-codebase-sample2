...
public partial class adm_ppv_module_descriptions : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (LoginManager.IsPagePermitted("adm_ppv_modules.aspx") == false)
        ...
        if (!IsPostBack)
        {
            ...
            Int32 nMenuID = 0;
            m_sMenu = TVinciShared.Menu.GetMainMenu(14, true, ref nMenuID, "adm_ppv_modules.aspx");
    ...
    protected void FillTheTableEditor(ref DBTableWebEditor theTable, string sOrderBy)
    {
        ...
        theTable += "select pcd.id as id,pcd.language_code3 as 'Language',pcd.description as 'Description',pcd.status,lcs.description as 'State' from ppv_descriptions pcd,lu_content_status lcs where lcs.id=pcd.status and pcd.status<>2 and ";
        ...
        theTable += " and ";
        theTable += ODBCWrapper.Parameter.NEW_PARAM("pcd.ppv_module_id", "=", int.Parse(Session["ppv_module_id"].ToString()));
        if (sOrderBy != "")
        {
            theTable += " order by ";
            theTable += sOrderBy;
        ...
        if (LoginManager.IsActionPermittedOnPage("adm_ppv_modules.aspx", LoginManager.PAGE_PERMISION_TYPE.EDIT))
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_ppv_module_descriptions_new.aspx", "Edit", "");
            linkColumn1.AddQueryStringValue("ppv_module_description_id", "field=id");
            theTable.AddLinkColumn(linkColumn1);
        }

        if (LoginManager.IsActionPermittedOnPage("adm_ppv_modules.aspx", LoginManager.PAGE_PERMISION_TYPE.REMOVE))
        {
            DataTableLinkColumn linkColumn = new DataTableLinkColumn("adm_generic_remove.aspx", "Delete", "STATUS=1;STATUS=3");
            linkColumn.AddQueryStringValue("id", "field=id");
            linkColumn.AddQueryStringValue("db", "pricing_connection");
            linkColumn.AddQueryStringValue("table", "ppv_descriptions");
        ...
        if (LoginManager.IsActionPermittedOnPage("adm_ppv_modules.aspx", LoginManager.PAGE_PERMISION_TYPE.PUBLISH))
        {
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        Session["ContentPage"] = "adm_ppv_modules.aspx";
        Session["LastContentPage"] = "adm_ppv_module_descriptions.aspx?ppv_module_id=" + Session["ppv_module_id"].ToString();
        theTable.Finish();
        theTable = null;
        return sTable;
    }
    ...
    {
        Response.Write(PageUtils.GetPreHeader() + ": PPM Module (" + Session["ppv_module_id"].ToString() + ") descriptions");
    }
}
