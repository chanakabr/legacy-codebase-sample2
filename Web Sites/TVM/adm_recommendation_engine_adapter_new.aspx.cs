...
public partial class adm_recommendation_engine_adapter_new : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["submited"] != null && Request.QueryString["submited"].ToString().Trim() == "1")
            {
                ...
                {
                    int.TryParse(Session["adapter_id"].ToString(), out pgid);
                }
    ...
    static private bool IsExternalIDExists(string extId, int pgid)
    {
        ...
        if (selectQuery.Execute("query", true) != null)
        {
            ...
            {
                result = true;
                int adapterId = ODBCWrapper.Utils.GetIntSafeVal(selectQuery, "ID", 0);
                string name = ODBCWrapper.Utils.GetStrSafeVal(selectQuery, "NAME", 0);
                Logger.Logger.Log("IsExternalIDExists", string.Format("id:{0}, name:{1}", adapterId, name), "recommendation_engines");
            }
        }
