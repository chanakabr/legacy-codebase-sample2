...
public partial class adm_discounts : System.Web.UI.Page
{
    ...
    protected void FillTheTableEditor(ref DBTableWebEditor theTable, string sOrderBy)
    {
        ...
        theTable += "select dc.is_active,dc.id as id,dc.ID as 'Discount ID',dc.CODE as 'Code',dc.Price as 'Amount',lc.name as 'Currency',dc.DISCOUNT_PERCENT as 'Percent',lwa.description as 'Validity',dc.WHENALGO_TIMES as 'Validity of discount delta',CONVERT(VARCHAR(10),dc.START_DATE, 104) as 'Start Date',CONVERT(VARCHAR(10),dc.End_DATE, 104) as 'End Date',dc.status,lcs.description as 'State' from discount_codes dc,lu_content_status lcs,lu_currency lc,lu_WHENALGO_TYPE lwa where lwa.id=dc.WHENALGO_TYPE and lc.id=dc.CURRENCY_CD and lcs.id=dc.status and dc.status<>2 and ";
        theTable += ODBCWrapper.Parameter.NEW_PARAM("dc.group_id", "=", nGroupID);
        if (sOrderBy != "")
        {
            theTable += " order by ";
            theTable += sOrderBy;
        ...
        theTable.AddOrderByColumn("Code", "dc.CODE");
        theTable.AddActivationField("discount_codes");
        theTable.AddHiddenField("is_active");

        /*
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.EDIT))
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_discounts_descriptions.aspx", "Descriptions", "");
            linkColumn1.AddQueryStringValue("discount_code_id", "field=id");
        ...
        */
        ...
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.EDIT))
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_discounts_new.aspx", "Edit", "");
            linkColumn1.AddQueryStringValue("discount_code_id", "field=id");
            theTable.AddLinkColumn(linkColumn1);
        }
        ...
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.REMOVE))
        {
            ...
            linkColumn.AddQueryStringValue("table", "discount_codes");
    ...
    public void UpdateOnOffStatus(string theTableName, string sID, string sStatus)
    {
        ...
        {
            log.ErrorFormat("Failed to set pricing settings invalidation key after discount code status change, key = {0}", invalidationKey);
        }
    }
}
