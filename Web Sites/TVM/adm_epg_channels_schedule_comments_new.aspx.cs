...
public partial class adm_epg_channels_schedule_comments_new : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString["program_id"] != null && Request.QueryString["program_id"].ToString() != "")
            {
                ...
                nOwnerGroupID = int.Parse(PageUtils.GetTableSingleVal("epg_channels_schedule", "group_id", int.Parse(Session["program_id"].ToString())).ToString());
            ...
            if (Request.QueryString["comment_type_id"] != null && Request.QueryString["comment_type_id"].ToString() != "")
            {
            ...
            if (Request.QueryString["program_comment_id"] != null && Request.QueryString["program_comment_id"].ToString() != "")
            {
                Session["program_comment_id"] = int.Parse(Request.QueryString["program_comment_id"].ToString());
            }
            else
            {
                Session["program_comment_id"] = 0;
            }
    ...
    {
        Response.Write(PageUtils.GetPreHeader() + ":" + PageUtils.GetTableSingleVal("epg_channels_schedule", "NAME", int.Parse(Session["program_id"].ToString())).ToString() + " Comment from type: ");
    }
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        object t = null; ;
        if (Session["program_comment_id"] != null && Session["program_comment_id"].ToString() != "" && int.Parse(Session["program_comment_id"].ToString()) != 0)
            t = Session["program_comment_id"];
        string sBack = "adm_epg_channels_schedule_comments.aspx?search_save=1&program_id=" + Session["program_id"].ToString() + "&comment_type_id=" + Session["comment_type_id"].ToString();
        DBRecordWebEditor theRecord = new DBRecordWebEditor("epg_comments", "adm_table_pager", sBack, "", "ID", t, sBack, "");
        ...
        dr_FILTRED_CONTENT_TEXT.Initialize("Filterd", "adm_table_header_nbg", "FormInput", "", false);
        ...
        DataRecordShortIntField dr_PROGRAMDIA_ID = new DataRecordShortIntField(false, 9, 9);
        dr_PROGRAMDIA_ID.Initialize("Group", "adm_table_header_nbg", "FormInput", "EPG_PROGRAM_ID", false);
        dr_PROGRAMDIA_ID.SetValue(Session["program_id"].ToString());
        theRecord.AddRecord(dr_PROGRAMDIA_ID);

        string sTable = theRecord.GetTableHTML("adm_epg_channels_schedule_comments_new.aspx?submited=1");
    ...
    {
        Int32 nCommentID = int.Parse(Session["program_comment_id"].ToString());

        string sContent = ODBCWrapper.Utils.GetTableSingleVal("epg_comments", "content_text", nCommentID).ToString();
