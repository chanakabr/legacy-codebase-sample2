...
public partial class adm_external_recommendation_search_new : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (LoginManager.IsPagePermitted("adm_external_recommendation_search_new.aspx") == false)
            LoginManager.LogoutFromSite("login.html");
        if (LoginManager.IsActionPermittedOnPage("adm_external_recommendation_search_new.aspx", LoginManager.PAGE_PERMISION_TYPE.EDIT) == false)
            LoginManager.LogoutFromSite("login.html");
    ...
    {
        Response.Write(PageUtils.GetPreHeader() + ": External Recommendaion Engine Search");
    }
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        string backUrl = "adm_external_recommendation_search_new.aspx?search_save=1";
        ...
        dr_recommendation_engine.Initialize("Recommendation Engine Provider", "adm_table_header_nbg", "FormInput", "SEARCH_RECOMMENDATION_ENGINE", false);
    ...
    public string changeItemStatus(string id, string action)
    {
        ...
        object channelEnrichmentsObject = PageUtils.GetTableSingleVal("groups", "SEARCH_RECOMMENDATION_ENGINE_ENRICHMENTS", groupId);
        long enrichments = 0;
    ...
    private void UpdateChannel(int groupId, long channelEnrichments)
    {
        ...
        updateQuery += ODBCWrapper.Parameter.NEW_PARAM("SEARCH_RECOMMENDATION_ENGINE_ENRICHMENTS", "=", channelEnrichments);
        updateQuery += "where ";
    ...
    public string initDualObj()
    {
        ...
        dualList.Add("pageName", "adm_external_recommendation_search_new.aspx");
        dualList.Add("withCalendar", false);

        log.Debug("External recommendation engine search - " + resultData.ToJSON());
    ...
    private long GetGroupEnrichments(int groupsId)
    {
        ...
        selectQuery += "select SEARCH_RECOMMENDATION_ENGINE_ENRICHMENTS from groups where ";
        ...
        if (selectQuery.Execute("query", true) != null)
        {
            ...
            if (count > 0)
            {
                value = ODBCWrapper.Utils.ExtractValue<long>(table.Rows[0], "SEARCH_RECOMMENDATION_ENGINE_ENRICHMENTS");
            }
        }
