...
public partial class adm_epg_pics_templates_new : System.Web.UI.Page
{
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        dr_ratios.SetSelectsQuery("select lur.ratio as 'txt', lur.id from  lu_pics_epg_ratios lur, groups g where g.id = " + LoginManager.GetLoginGroupID() + " and lur.id = g.ratio_id" + " UNION select lur.ratio as 'txt', lur.id from lu_pics_epg_ratios lur, group_ratios gr where gr.group_id = " + LoginManager.GetLoginGroupID() + " and gr.ratio_id = lur.id and gr.status = 1");
