...
public partial class adm_groups_language_display : System.Web.UI.Page
{
    ...
    {
        Response.Write(PageUtils.GetPreHeader() + ": Group Language Display");
    }
    ...
    public string GetPageContent(string sOrderBy, string sPageNum)
    {
        ...
        theTable += @"select gldn.ID, gldn.DISPLAY_NAME as 'Display Name', ll.name as 'Name', ll.CODE3 as 'Code3',  gldn.status
                      from dbo.groups_language_display_name gldn
                      join dbo.lu_languages ll on (gldn.language_id=ll.id)
                      where gldn.status<>2 and ";
        theTable += ODBCWrapper.Parameter.NEW_PARAM("gldn.group_id", "=", LoginManager.GetLoginGroupID());
        if (sOrderBy != "")
        {
            theTable += " order by ";
            theTable += sOrderBy;
        ...
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_groups_language_display_new.aspx", "Edit", "");
        ...
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.REMOVE))
        {
            ...
            linkColumn.AddQueryStringValue("table", "groups_language_display_name");
