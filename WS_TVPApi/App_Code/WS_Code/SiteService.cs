    ...
    public class SiteService : System.Web.Services.WebService, ISiteService
    {
        ...
        public string GenerateDeviceToken(InitializationObject initObj, string appId)
        {
            ...
            if (groupID > 0)
            {
                try
                {
                    response = AuthorizationManager.GenerateDeviceToken(initObj.UDID, groupID, appId);
                }
                catch (Exception ex)
                {
                    HttpContext.Current.Items.Add("Error", ex);
        ...
        public APIToken ExchangeDeviceToken(InitializationObject initObj, string appId, string appSecret, string deviceToken)
        {
            ...
            if (groupID > 0)
            {
                try
                {
                    response = AuthorizationManager.ExchangeDeviceToken(initObj.UDID, groupID, appId, appSecret, deviceToken);
                }
                catch (Exception ex)
                {
                    HttpContext.Current.Items.Add("Error", ex);
        ...
        public APIToken RefreshAccessToken(InitializationObject initObj, string appId, string appSecret, string refreshToken)
        {
            ...
            if (groupID > 0)
            {
                try
                {
                    response = AuthorizationManager.RefreshAccessToken(initObj.UDID, groupID, appId, appSecret, refreshToken);
                }
                catch (Exception ex)
                {
                    HttpContext.Current.Items.Add("Error", ex);
