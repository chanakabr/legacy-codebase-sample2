...
namespace Kaltura
{
    public class User : KalturaUsers
    {
            ...
            ref string sessionId, ref string ip, ref string deviceId, ref bool preventDoubleLogin, ref List<KeyValuePair> keyValueList)
        {
            ...
            if (keyValueList.Where(x => x.key.ToLower() == "testcase" && x.value.ToLower() == "otp").Count() == 0)
            {
                    ...
                    if (domainStatusResponse != null && domainStatusResponse.Status != null && domainStatusResponse.Status.Code == (int)eResponseStatus.OK)
                    {
                        ...
                        try
                        {
                            ...
                            if (pinResponse != null && pinResponse.resp != null && pinResponse.resp.Code == (int)eResponseStatus.OK)
                            {
                                ...
                                request.m_sTemplateName = "pincode_203.html"; // get this from where 

                                request.values = new List<KeyValuePair>();
                                request.values.Add(new KeyValuePair() { key = "PASSWORD", value = password });
                                request.values.Add(new KeyValuePair() { key = "PINCODE", value = pinResponse.pinCode });
