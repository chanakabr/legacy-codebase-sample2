    ...
    public class ImporterImpl
    {
        ...
        public static void ClearImages(long nGroupID, Int32 nMediaID)
        {
            ...
            if (!WS_Utils.IsGroupIDContainedInConfig(nGroupID, ApplicationConfiguration.UseOldImageServer.Value, ';'))
            {
                ...
                {
                    int parentGroupId = UtilsDal.GetParentGroupID((int)nGroupID);
        ...
        public static bool ProcessCategoryItems(XmlDocument theItem, ref string sCoGuid, ref Int32 nChannelID, ref string sErrorMessage, Int32 nGroupID)
        {
            ...
            //get value from tcm
            int rootCategoryID = WS_Utils.GetTcmIntValue("ROOT_CATEGORY_ID");
            ImporterImpDAL.StartCategoriesTransaction(rootCategoryID);
            ...
            try
            {
                ...
                for (int i = 0; i < deserializedChannels.export.Length; ++i)
                {
                    ...
                    if (deserializedChannels.export[i] != null)
                    {
                        ...
                        UpdateChannelIndex(LoginManager.GetLoginGroupID(), new List<int>() { channelID }, eAction.Update);
                    }
                }
        ...
        public static bool InsertNewEPGMultiPic(string epgIdentifier, int picID, int ratioID, int nGroupID, int nChannelID)
        {
            try
            {
                bool result = EpgDal.InsertNewEPGMultiPic(epgIdentifier, picID, ratioID, nGroupID, nChannelID);
                return result;
            }
            catch (Exception ex)
            {
        ...
        public static Int32 DownloadEPGPicToUploader(string sThumb, string sName, Int32 nGroupID, Int32 nEPGSchedID, int nChannelID, int ratioID)
        {
            ...
            if (nPicID == 0)
            {
                ...
                lock (m_sLocker)
                {
                    sUploadedFile = ImageUtils.DownloadWebImage(sThumb, sBasePath);
                }
                if (sUploadedFile == "")
                    return 0;
                ...
                string sPicBaseName = ImageUtils.GetDateImageName();
        ...
        public static Int32 DownloadEPGPicToQueue(string sThumb, string sName, Int32 nGroupID, Int32 nEPGSchedID, int nChannelID, int ratioID)
        {
            ...
            if (picId == 0)
            {
                ...
                string sPicNewName = ImageUtils.GetDateImageName();
        ...
        static protected Int32 DownloadEPGPic(string sThumb, string sName, Int32 nGroupID, Int32 nEPGSchedID, string sMainLang)
        {
            ...
            if (nPicID == 0)
            {
                ...
                if (selectQuery.Execute("query", true) != null)
                {
                    ...
                    ImageUtils.ResizeImageAndSave(sBasePath + "/pics/" + sUploadedFile, sBasePath + "/pics/" + nGroupID.ToString() + "/" + sPicBaseName + "_tn" + sUploadedFileExt, 90, 65, true);
                    UploadQueue.UploadQueueHelper.AddJobToQueue(nGroupID, sPicBaseName + "_tn" + sUploadedFileExt);

                    ImageUtils.RenameImage(sBasePath + "/pics/" + sUploadedFile, sBasePath + "/pics/" + nGroupID.ToString() + "/" + sPicBaseName + "_full" + sUploadedFileExt);
                    ...
                    for (int nI = 0; nI < nCount; nI++)
                    {
                        ...
                        ImageUtils.ResizeImageAndSave(sBasePath + "/pics/" + sUploadedFile, sTmpImage1, int.Parse(sWidth), int.Parse(sHeight), true);
        ...
        public static Int32 DownloadPic_old(string sPic, string sMediaName, Int32 nGroupID, Int32 nMediaID, string sMainLang, string sPicType, bool bSetMediaThumb, int ratioID)
        {
            ...
            object oPicsBasePath = PageUtils.GetTableSingleVal("groups", "PICS_REMOTE_BASE_URL", nGroupID);
            ...
            {
                doesPicExist = false;
                string sUploadedFile = ImageUtils.DownloadWebImage(sPic, sBasePath);
                if (sUploadedFile == "")
                {
                    sUploadedFile = ImageUtils.DownloadWebImage(sPicsBasePath + "/" + sPic, sBasePath);
                    if (sUploadedFile == "")
                    {
                        return 0;
                    }
                ...
                if (ratioID > 0)
                {
                    sPicBaseName = ImageUtils.GetDateImageName(nMediaID);
                }
                ...
                bool bExists = File.Exists(sTmpImage);
                Int32 nAdd = 0;
                while (bExists)
                {
                ...
                if (selectQuery.Execute("query", true) != null)
                {
                    ...
                    for (int nI = 0; nI < nCount1; nI++)
                    {
                        ...
                        ImageUtils.ResizeImageAndSave(sBasePath + "/pics/" + sUploadedFile, sTmpImage1, int.Parse(sWidth), int.Parse(sHeight), bCrop, bOverride);
            ...
            if (nPicID != 0)
            {
                if (doesPicExist)
                {
                    ...
                    if (selectQuery.Execute("query", true) != null)
                    {
                        ...
                        ImageUtils.ResizeImageAndSave(sBasePath + "/pics/" + sUploadedFile, sBasePath + "/pics/" + nGroupID + "/" + sPicBaseName + "_tn" + sUploadedFileExt, 90, 65, true, true);
                        log.Debug("Re cropping: " + sBasePath + "/pics/" + sPicBaseName + "_tn" + sUploadedFileExt);
                        UploadQueue.UploadQueueHelper.AddJobToQueue(nGroupID, sPicBaseName + "_tn" + sUploadedFileExt);

                        ImageUtils.RenameImage(sBasePath + "/pics/" + sUploadedFile, sBasePath + "/pics/" + nGroupID + "/" + sPicBaseName + "_full" + sUploadedFileExt);
                        ...
                        for (int nI = 0; nI < nCount1; nI++)
                        {
                            ...
                            ImageUtils.ResizeImageAndSave(sBasePath + "/pics/" + sUploadedFile, sTmpImage1, int.Parse(sWidth), int.Parse(sHeight), bCrop, true);
        ...
        public static int DownloadPicToImageServer(string pic, string assetName, int groupId, int assetId, string mainLang, bool setMediaThumb, int ratioId, eAssetImageType assetImageType, bool isAsync = true, int? updaterId = null)
        {
            ...
            {
                baseUrl = ImageUtils.GetDateImageName();
                picId = CatalogDAL.InsertPic(groupId, assetName, pic, baseUrl, ratioId, assetId, assetImageType);
            }
        ...
        static private string GetBasePath(int nGroupID)
        {
            ...
            if (!string.IsNullOrEmpty(WS_Utils.GetTcmConfigValue(key)))
            {
            ...
            try
            {
                ...
                else
                {
                    try
                    {
                        ...
                        {
                            sBasePath = AppDomain.CurrentDomain.BaseDirectory;
                        }
                        else
                            sBasePath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
                    }
                    catch (Exception ex)
                    {
                        log.Error("", ex);
                    ...
                    for (int mediaIndex = 0; mediaIndex < mediaItems.Count; mediaIndex++)
                    {
                        ...
                        try
                        {
                            ...
                            if (ProcessItem(mediaItems[mediaIndex], ref sCoGuid, ref nMediaID, ref sErrorMessage, nGroupID, ref isActive, ref ingestAssetStatus, nParentGroupID, out action))
                            {
                                ...
                                if (UpdateIndex(new List<int>() { nMediaID }, nParentGroupID, action))
                                {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(sWSURL))
                {
                    ...
                    foreach (string sEndPointAddress in arrAddresses)
                    {
                        try
                        {
                            bUpdate = Core.Catalog.Module.UpdateChannel(nGroupId, nChannelID);
        ...
        {
            string sLuceneURL = WS_Utils.GetTcmConfigValue("LUCENE_WCF_" + nGroupID);  //TCM not relevant anymore 
            try
            {
                DataTable dt = ImporterImpDAL.Get_LuceneUrl(nGroupID);
                if (dt != null)
                {
        ...
        private static string GetCatalogUrlByParameters(int groupId, eObjectType? objectType, eAction? action)
        {
            ...
            try
            {
                catalogURL = ImporterImpDAL.Get_CatalogUrlByParameters(groupId, objectType, action);
            ...
            string timezone, string imageUrl, bool includeMail, string mailTemplate, string mailSubject, bool includeSMS, ref int id)
        {
            ...
            try
            {
                ...
                response = Core.Notification.Module.AddMessageAnnouncement(nParentGroupID, announcement);
            ...
            string timezone, string imageUrl, bool includeMail, string mailTemplate, string mailSubject, bool includeSms)
        {
            try
            {
                ...
                announcement.MailSubject = mailSubject;
                announcement.IncludeSms = includeSms;
                MessageAnnouncementResponse response = Core.Notification.Module.UpdateMessageAnnouncement(nParentGroupID, id, announcement);
                return response.Status;
            }
            catch (Exception)
            {
        ...
        public static bool UpdateMessageAnnouncementStatus(int groupID, int id, bool status)
        {
            try
            {
                ...
                int nParentGroupID = UtilsDal.GetParentGroupID(groupID);
                Status response = Core.Notification.Module.UpdateMessageAnnouncementStatus(nParentGroupID, id, status);
        ...
        static private void UpdateNotification(object val)
        {
            ...
            int nGroupID = vals[0];
            int mediaID = vals[1];
            bool notificationResult = UpdateNotificationRequest(nGroupID, mediaID); //(LoginManager.GetLoginGroupID(), nID);
        }
        ...
        static private bool UpdateNotificationRequest(int groupid, int nMediaID)
        {
            ...
            try
            {
                ...
                int nParentGroupID = UtilsDal.GetParentGroupID(groupid);
                bUpdate = Core.Notification.Module.AddNotificationRequest(nParentGroupID, string.Empty, NotificationTriggerType.FollowUpByTag, nMediaID);
            }
            catch (Exception ex)
            {
        ...
        public static ApiObjects.Response.Status SetMessageTemplate(int groupID, ref ApiObjects.Notification.MessageTemplate messageTemplate)
        {
            ...
            try
            {
                ...
                response = Core.Notification.Module.SetMessageTemplate(nParentGroupID, wcfMessageTemplate);
                if (response != null && response.Status.Code == (int)eResponseStatus.OK)
                {
        ...
        public static Status AddEngagement(int groupId, ref Engagement engagement)
        {
            ...
            try
            {
                ...
                Engagement wcfEngagement = new Engagement()
                {
                ...
                response = Core.Notification.Module.AddEngagement(nParentGroupID, wcfEngagement);
                return response.Status;
            }
            catch
            {
        ...
        public static Status SetEngagementAdapterConfiguration(int groupId, int engagementAdapterId)
        {
            ...
            {
                int nParentGroupID = UtilsDal.GetParentGroupID(groupId);
                return Core.Notification.Module.SetEngagementAdapterConfiguration(nParentGroupID, engagementAdapterId);
            }
            catch
            {
                return new Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString());
            ...
            try
            {
                ...
                if (lMediaIds != null && lMediaIds.Count > 0 && nParentGroupID > 0)
                {
                    ...
                    try
                    {
                        ...
                        log.DebugFormat("Update index {0} in catalog '{1}'", sInfo);
                    }
                    catch (Exception ex)
                    {
                        log.Error(string.Format("Couldn't update catalog due to the following error: {0}", ex.Message), ex);
                    }
                }
            }
            catch (Exception ex)
            {
                log.Error("Error TviniciImporter UpdateIndex", ex);
            }
        ...
        private static void UpdateRemindersfEPGs(List<ulong> epgIds, int groupId)
        {
            try
            {
                ...
                Core.Notification.Module.HandleEpgEvent(groupId, epgIds);
        ...
        public static void UpdateRecordingsOfEPGs(List<ulong> epgIds, int groupId, eAction action, bool isCalledFromTvm = false)
        {
            try
            {
        ...
        public static bool UpdateEpgChannelIndex(List<int> epgChannels, int groupID, eAction action)
        {
            bool isUpdateIndexSucceeded = false;
        ...
        public static bool UpdateFreeFileTypeOfModule(int groupId, int moduleId)
        {
            ...
            {
                DataTable mediaIds = ImporterImpDAL.GetMediasByPPVModuleID(groupId, moduleId, 0);
                while (mediaIds != null && mediaIds.Rows != null)
                {
                    ...
                    if (mediaIDsToUpdate != null && mediaIDsToUpdate.Count > 0)
                    {
                        ...
                        if (result)
                        {
                            ...
                            mediaIds = ImporterImpDAL.GetMediasByPPVModuleID(groupId, moduleId, lastMediaID);
                        }
                    }
                }
            }
        ...
        public static Dictionary<string, int> GetAmountOfSubscribersPerAnnouncement(int groupID)
        {
            ...
            {
                int nParentGroupID = UtilsDal.GetParentGroupID(groupID);
                response = Core.Notification.Module.GetAmountOfSubscribersPerAnnouncement(nParentGroupID);
                if (response == null)
                {
        ...
        public static ApiObjects.Response.Status SetTopicSettings(int groupId, int id, eTopicAutomaticIssueNotification topicAutomaticIssueNotification)
        {
            ...
            try
            {
                ...
                status = Core.Notification.Module.UpdateAnnouncement(nParentGroupID, id, topicAutomaticIssueNotification);
        ...
        public static bool DeleteAnnouncement(int groupId, int announcementId)
        {
            ...
            {
                int nParentGroupID = UtilsDal.GetParentGroupID(groupId);
                var response = Core.Notification.Module.DeleteAnnouncement(nParentGroupID, announcementId);
                if (response == null || response.Code != (int)eResponseStatus.OK)
                {
