...
namespace Core.ConditionalAccess
{
    public abstract class BaseConditionalAccess
    {
        ...
        protected abstract bool UpdatePurchaseIDInBilling(long purchaseID, long billingRefTransactionID);

        protected abstract bool HandleChargeUserForSubscriptionBillingSuccess(string sSiteGUID, int domianID, Subscription theSub,
        ...
        protected abstract bool HandleChargeUserForCollectionBillingSuccess(string sSiteGUID, int domianID, Collection theCol,
            double dPrice, string sCurrency, string sCouponCode, string sUserIP, string sCountryCd, string sLanguageCode,
            ...
            string sPaymentMethod, string sDateOfPurchase, string sRecNumner, double dPrice, string sCurrency, Int32 nGroupID)
        {
            ...
            UserResponseObject uObj = Core.Users.Module.GetUserData(nGroupID, sUserGUID, string.Empty);
        ...
        protected internal void WriteToUserLog(string sSiteGUID, string sMessage)
        {
            try
            {
                Core.Users.Module.WriteLog(m_nGroupID, sSiteGUID, sMessage, "Conditional access module");
            }
            catch (Exception ex)
            {
        ...
        protected virtual BillingResponse HandleCellularChargeUser(int nGroupId, string sSiteGuid, double dPrice, string sCurrency, string sUserIP, string sCustomData,
                                                                                 int nPaymentNumber, int nNumOfPayments, string sExtraParams, bool bIsDummy, bool bIsEntitledToPreviewModule)
        {
            if (!bIsDummy && !bIsEntitledToPreviewModule)
            {
                ...
                //Cellular_ChargeUser
                return Billing.Module.Cellular_ChargeUser(nGroupId, sSiteGuid, dPrice, sCurrency, sUserIP, sCustomData, 1, 1, sExtraParams);

            }
            else
            {
                return Billing.Module.CC_DummyChargeUser(nGroupId, sSiteGuid, dPrice, sCurrency, sUserIP, sCustomData, 1, 1, sExtraParams);
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        thePrePaidModule = Pricing.Module.GetPrePaidModuleData(m_nGroupID, int.Parse(sPrePaidModuleCode), sCountryCd, sLANGUAGE_CODE, sDEVICE_NAME);
                                ...
                                if (theReason == PriceReason.ForPurchase && p.m_dPrice > 0 || bDummy == true)
                                {
                                    if (bDummy || (p.m_dPrice == dPrice && p.m_oCurrency.m_sCurrencyCD3 == sCurrency))
                                    {
                                        ...
                                        if (p.m_dPrice != 0 || bDummy)
                                        {
                                            ...
                                            //customdata id
                                            if (!bDummy)
                                                ret = Billing.Module.CC_ChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData, 1, 1, sExtraParameters, string.Empty, string.Empty);
                                            else
                                                ret = Billing.Module.CC_DummyChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData, 1, 1, sExtraParameters);
                                        }
                                        if (ret.m_oStatus == BillingResponseStatus.Success)
                                        {
                                            ...
                                            if (sReciept != "")
                                            {
                                                ...
                                                try
                                                {
                                                    ...
                                                    if (!bDummy)
                                                    {
                                                        string sDigits = Billing.Module.CC_GetUserCCDigits(m_nGroupID, sSiteGUID);
                                                    ...
                                                    Api.Module.SendMailTemplate(m_nGroupID, sMailReq);
                    ...
                    else
                    {
                        ...
                        if (theReason == PriceReason.ForPurchase)
                        {
                            if (p != null && dPrice > 0)
                            {
                                ...
                                if (p.m_dPrice != 0)
                                {
                                    InAppRes = Billing.Module.InApp_ChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData, 1, nRecPeriods, ReceiptData);
                                }
                    ...
                    else
                    {
                        ...
                        else if (theSub != null)
                        {
                            ...
                            if (dPrice != 0)
                            {
                                ...
                                ret = Billing.Module.CC_ChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData, nPaymentNumber, nRecPeriods, sExtraParams, string.Empty, string.Empty);
                            }
                        }
                        if (ret.m_oStatus == BillingResponseStatus.Success)
                        {
                    ...
                    else
                    {
                        ...
                        else if (theSub != null && theSub.m_oSubscriptionUsageModule != null)
                        {
                            if (dPrice != 0)
                            {
                                ...
                                ret = Billing.Module.InApp_ReneweInAppPurchase(m_nGroupID, sSiteGUID, dPrice, sCurrency, sCustomData, nPaymentNumber, nRecPeriods, nInAppTransactionID);
                            }
                        }
        ...
        */
            ...
            if (ExistUser != null && ExistUser.m_RespStatus == ResponseStatus.OK)
            {
                try
                {
                    ...
                    var renewDetails = new RenewDetails()
                    {
                        ...
                        CustomData = sCustomData,
                        EndDate = dtCurrentEndDate,
                        MaxVLCOfSelectedUsageModule = nMaxVLCOfSelectedUsageModule,
                        //RecurringData = ConditionalAccessDAL.GetRecurringRenewDetails(nPurchaseID)
                        RecurringData = new RecurringRenewDetails()
                        {
                            TotalNumOfRenews = nTotalPaymentsNumber,
                            IsPurchasedWithPreviewModule = bIsPurchasedWithPreviewModule,
                            CouponCode = string.Empty
                        }
                    };

                    GetMultiSubscriptionUsageModule(renewDetails, userIP, ref nRecPeriods, ref bIsMPPRecurringInfinitely, theSub);

                    nMaxVLCOfSelectedUsageModule = renewDetails.MaxVLCOfSelectedUsageModule;
                    dPrice = renewDetails.Price;
                    sCustomData = renewDetails.CustomData;
                    sCurrency = renewDetails.Currency;
                }
                catch (Exception ex)
                {
                                                                ...
                                                                Subscription subscription, ref UnifiedBillingCycle unifiedBillingCycle, int groupId = 0, bool ignoreUnifiedBillingCycle = true, bool isRenew = true)
        {
            ...
            try
            {
                UsageModule AppUsageModule = GetAppropriateMultiSubscriptionUsageModule(subscription, renewDetails);
                ...
                renewDetails.Price = 0;
                Currency oCurrency = null;
                renewDetails.Currency = "n/a";
                ...
                if (price == null)
                {
                    log.ErrorFormat("PriceCode is null for priceCodeId: {0}, previousPurchaseCurrencyCode: {1}", AppUsageModule.m_pricing_id,
                                                        !string.IsNullOrEmpty(renewDetails.PreviousPurchaseCurrencyCode) ? renewDetails.PreviousPurchaseCurrencyCode : string.Empty);
                    return isSuccess;
                }
                ...
                {
                    renewDetails.Price = clonedPrice.m_oPrise.m_dPrice;
                ...
                {
                    switch (renewDetails.RecurringData.Compensation.CompensationType)
                    {
                        case CompensationType.Percentage:
                            renewDetails.Price = renewDetails.Price - (renewDetails.Price * renewDetails.RecurringData.Compensation.Amount / 100);
                            break;
                        case CompensationType.FixedAmount:
                            renewDetails.Price = Math.Max(renewDetails.Price - renewDetails.RecurringData.Compensation.Amount, 0);
                            break;
                        default:
                            break;
                    }
                ...
                renewDetails.MaxVLCOfSelectedUsageModule = AppUsageModule.m_tsMaxUsageModuleLifeCycle;

                if (isRenew)
                {
                    renewDetails.CustomData =
                        GetCustomDataForMPPRenewal(renewDetails, subscription, AppUsageModule, clonedPrice, subscription.m_SubscriptionCode, userIp, isPartialPrice);
                }
                isSuccess = true;
            }
            catch (Exception ex)
                    ...
                    else
                    {
                        ...
                        else if (theSub != null && theSub.m_oSubscriptionUsageModule != null)
                        {
                            ...
                            if (dPrice != 0)
                            {
                                ...
                                //customdata id
                                ret = Billing.Module.DD_ChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData, nPaymentNumber, nRecPeriods, nPurchaseID.ToString(), nBillingMethod);
        ...
        private UsageModule GetAppropriateMultiSubscriptionUsageModule(Subscription thesub, RenewDetails renewDetails)
        {
            UsageModule u = null;
            int paymentNumber = renewDetails.PaymentNumber;
            if (!renewDetails.EndDate.HasValue)
            {
                DataRow dr = ODBCWrapper.Utils.GetTableSingleRowColumnsByParamValue("subscriptions_purchases", "ID", renewDetails.PurchaseId.ToString(), new List<string>() { "END_DATE" }, "CA_CONNECTION_STRING");
                if (dr == null)
                {
                    log.ErrorFormat("Failed to get subscriptions_purchases end date. purchaseId = {0}", renewDetails.PurchaseId);
                    return null;
                }

                renewDetails.EndDate = ODBCWrapper.Utils.GetDateSafeVal(dr, "END_DATE");
            }

            DateTime tempDate = renewDetails.EndDate.Value;

            if (thesub.m_nNumberOfRecPeriods != 0)
            {
                int npm = paymentNumber % thesub.m_nNumberOfRecPeriods;
                if (npm != 0)
                {
                    paymentNumber = npm;
                }
                ...
                {
                    paymentNumber = thesub.m_nNumberOfRecPeriods;
                }
            ...
            if (thesub.m_MultiSubscriptionUsageModule != null)
            {
                ...
                for (int i = 0; i < thesub.m_MultiSubscriptionUsageModule.Length; i++)
                {
                    ...
                    if (/*i == 0 && uList[i].m_is_renew == 0*/
                        IsSkipOnFirstUsageModule(i,
                                                 uList[i].m_is_renew == 1,
                                                 renewDetails.RecurringData.TotalNumOfRenews,
                                                 renewDetails.NumOfPayments,
                                                 renewDetails.RecurringData.IsPurchasedWithPreviewModule))
                    {
                        /*
                    ...
                    else if (paymentNumber <= totalperiod)
                    {
                        if (renewDetails.EndDate < tempDate)
                        {
                            u = thesub.m_MultiSubscriptionUsageModule[i];
                            break;
                        }
        ...
        protected bool isDevicePlayValid(string sSiteGUID, string sDEVICE_NAME, ref Domain userDomain)
        {
            ...
            UserResponseObject userRepObj = Core.Users.Module.GetUserData(m_nGroupID, sSiteGUID, string.Empty);
            if (userRepObj != null && userRepObj.m_user != null && userRepObj.m_RespStatus == ResponseStatus.OK)
            {
                ...
                if (domainID != 0)
                {
                    var res = Core.Domains.Module.GetDomainInfo(m_nGroupID, domainID);
                    if (res != null)
                    {
                    ...
                    else
                    {
                        ...
                        if (theReason == PriceReason.ForPurchase)
                        {
                            if (p.m_dPrice == dPrice && p.m_oCurrency.m_sCurrencyCD3 == sCurrency)
                            {
                                ...
                                if (relevantSub != null)
                                {
                                    ret = Billing.Module.SMS_SendCode(m_nGroupID, sSiteGUID, sCellPhone, sCustomData, sExtraParameters);
        ...
        {
            List<string> usersList = Core.Domains.Module.GetDomainUserList(m_nGroupID, nDomainID);
        ...
        internal void IsCancellationWindow(ref UsageModule oUsageModule, string sAssetCode, DateTime dCreateDate, ref bool bCancellationWindow, eTransactionType transaction)
        {
            ...
            oUsageModule = Pricing.Module.GetUsageModule(m_nGroupID, sAssetCode, enumPricing);
            ...
            string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME)
        {
            ...
            try
            {
                ...
                ret = Billing.Module.SMS_CheckCode(m_nGroupID, sSiteGUID, sCellPhone, sSMSCode, sRefference);
                if (ret.m_sRecieptCode != "")
                {
                    if (ret.m_oStatus == BillingResponseStatus.Success)
                    {
                        ...
                        thePPVModule = Pricing.Module.GetPPVModuleData(m_nGroupID, sPPVCode, sCountryCd, sLANGUAGE_CODE, sDEVICE_NAME);
                    ...
                    else
                    {
                        ...
                        if ((ePriceReason == PriceReason.ForPurchase || (ePriceReason == PriceReason.SubscriptionPurchased && oPrice.m_dPrice > 0) || bDummy) && ePriceReason != PriceReason.NotForPurchase)
                        {
                            if (bDummy || (oPrice.m_dPrice == dPrice && oPrice.m_oCurrency.m_sCurrencyCD3 == sCurrency))
                            {
                                ...
                                oResponse = HandleCCChargeUser(sSiteGUID, dPrice, sCurrency, sUserIP, sCustomData,
                                    1, 1, sExtraParameters, sPaymentMethodID, sEncryptedCVV, bDummy, false);
            ...
            string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME, string sOverrideEndDate)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    oModules = Pricing.Module.GetPPVModuleListForMediaFiles(m_nGroupID, nMediaFiles, sCountryCd, sLANGUAGE_CODE, sDEVICE_NAME);
                    ...
                    else
                    {
                        ...
                        if (!string.IsNullOrEmpty(sCampaignCode))
                        {
                            ...
                            relevantCamp = Pricing.Module.GetCampaignData(m_nGroupID, nCampaignCode);
                        }
                        if (thePPVModule != null)
                        {
                            Price p = Utils.GetMediaFileFinalPriceForNonGetItemsPrices(nMediaFileID, thePPVModule, sSiteGUID, sCouponCode, m_nGroupID, ref theReason, ref relevantSub, ref relevantCol, ref relevantPP, sCountryCd, sLANGUAGE_CODE, sDEVICE_NAME);
    ...
    string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME)
                                ...
                                if (theReason == PriceReason.ForPurchase || (theReason == PriceReason.SubscriptionPurchased && p.m_dPrice > 0))
                                {
                                    if (p.m_dPrice == dPrice && p.m_oCurrency.m_sCurrencyCD3 == sCurrency)
                                    {
                                        if (p.m_dPrice != 0)
                                        {
                                            ...
                                            //customdata id
                                            ret.m_sRecieptCode = Billing.Module.CC_GetPopupURL(m_nGroupID, dPrice, sCurrency, "PPV Item", sCustomData, sPaymentMethod, sExtraParams);
                                            ret.m_oStatus = BillingResponseStatus.Success;
                        ...
                        if (p != null && p.m_dPrice == dPrice && p.m_oCurrency.m_sCurrencyCD3 == sCurrency)
                        {
                            if (p.m_dPrice != 0)
                            {
                                ...
                                if (sExtraParameters.StartsWith("&") == false)
                                    sExtraParameters = "&" + sExtraParameters;
                                sExtraParams += sExtraParameters;
                                ret.m_sRecieptCode = Billing.Module.CC_GetPopupURL(m_nGroupID, dPrice, sCurrency, "Subscription", sCustomData, sPaymentMethod, sExtraParams);
                                ret.m_oStatus = BillingResponseStatus.Success;
                                ret.m_sStatusDescription = "PopUp URL";
                                    ...
                                    case eBundleType.SUBSCRIPTION:
                                        {
                                            ret = ExecuteCCSubscriprionPurchaseFlow(theBundle as Subscription, sBundleCode, sSiteGUID, uObj.m_user.m_domianID, dPrice, sCurrency, sCouponCode,
                                                                        ...
                                                                        sPaymentMethodID, sEncryptedCVV, p);
                                            break;
                                        }
                                    case eBundleType.COLLECTION:
                                        {
                                                                     ...
                                                                     string ip, string currencyCode = null, BlockEntitlementType blockEntitlement = BlockEntitlementType.NONE, bool isDownloadPlayContext = false)
        {
                ...
                if (oModules != null && oModules.Length > 0)
                {
                    ...
                    for (int i = 0; i < oModules.Length; i++)
                    {
                        ...
                        if (ppvModules.Length > 0 && itemPriceCont.Count == 0 && !isUserSuspended)
                        {
                            ...
                            {
                                var tempItemPriceContainer = new ItemPriceContainer();
                                // in case ppv to media file connection has expired and no other valid ppv exists on the media file
                                if (!hasValidPpv && theLowestReason == PriceReason.UnKnown)
                                {
                                    tempItemPriceContainer = GetFreeItemPriceContainer();
                                }
                                else
                                {
            ...
            catch (Exception ex)
            {
                ...
                sb.Append(String.Concat(" Coupon Code: ", couponCode));
                sb.Append(String.Concat(" OnlyLowest: ", onlyLowest.ToString().ToLower()));
                if (mediaFiles != null && mediaFiles.Length > 0)
                {
                    sb.Append(" Media Files: ");
                    for (int i = 0; i < mediaFiles.Length; i++)
                    {
                        sb.Append(String.Concat(mediaFiles[i], " "));
                    }
                }
                else
                {
        ...
        {
            var freeContainer = new ItemPriceContainer
            {
                m_PriceReason = PriceReason.Free,
                m_oPrice = new Price()
                {
                    m_dPrice = 0.0
                }
        ...
        */
        ...
        private TransactionHistoryContainer GetBillingTransactionContainerFromDataRow(DataRow dr, bool shouldGetExtendedParams)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(sPrePaidCode))
                {
                    ...
                    thePrePaid = Pricing.Module.GetPrePaidModuleData(m_nGroupID, int.Parse(sPrePaidCode), string.Empty, string.Empty, string.Empty);
                ...
                else
                {
                    ...
                    if (pm == ePaymentMethod.CreditCard || pm == ePaymentMethod.Visa || pm == ePaymentMethod.MasterCard)
                    {
                        res.m_sPaymentMethodExtraDetails = sLAST_FOUR_DIGITS;
                    }

                    if (pm == ePaymentMethod.SMS || pm == ePaymentMethod.M1)
                    {
        ...
        /// GetI tems Prices
        /// </summary>
        public virtual MediaFileItemPricesContainer[] GetItemsPrices(Int32[] nMediaFiles, string sUserGUID, bool bOnlyLowest, string sLANGUAGE_CODE, string sDEVICE_NAME, string sClientIP = null, BlockEntitlementType blockEntitlement = BlockEntitlementType.NONE)
        {
            return GetItemsPrices(nMediaFiles, sUserGUID, "", bOnlyLowest, sLANGUAGE_CODE, sDEVICE_NAME, sClientIP, null, blockEntitlement);
        }
        /// <summary>
        ...
        protected internal virtual void HandleCouponUses(int subscriptionId, string userId, int mediaFileId, string couponCode, bool isFromPurchase, int collectionId, long domainId)
        {
            if (string.IsNullOrEmpty(couponCode) || (mediaFileId == 0 && subscriptionId == 0 && collectionId == 0)) { return; }

            if (isFromPurchase == false)
            {
                double dPercent = Utils.GetCouponDiscountPercent(m_nGroupID, couponCode, domainId);
            ...
            {
                log.Error(string.Format("An Exception was occurred in HandleCouponUses. couponCode:{0}.", couponCode), ex);
            }
        }
        ...
        /// Handle Coupon Uses
        /// </summary>
        protected internal virtual void HandleCouponUses(Subscription relevantSub, string sPPVModuleCode, string sSiteGUID, double dPrice, string sCurrency, Int32 nMediaFileID, string sCouponCode,
                                                         string sUserIP, string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME, bool bFromPurchase, int nPrePaidCode, Int32 relevantCollection,
                                                         long domainId)
        {
            Int32 nSubCode = 0;
            ...
            HandleCouponUses(nSubCode, sSiteGUID, nMediaFileID, sCouponCode, bFromPurchase, relevantCollection, domainId);
        }
    ...
    string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME, string sOverrideEndDate)
            ...
            if (selectQuery.Execute("query", true) != null)
            {
                ...
                for (int i = 0; i < nCount; i++)
                {
                    ...
                    if (dLastCredit != dCredit)
                    {
                        ...
                        if (selectQueryE.Execute("query", true) != null)
                        {
                            ...
                            for (int j = 0; j < nCount1; j++)
                            {
                                ...
                                string sWSPass = string.Empty;
                                Utils.GetWSCredentials(m_nGroupID, eWSModules.PRICING, ref sWSUserName, ref sWSPass);
                                thePrePaid = Pricing.Module.GetPrePaidModuleData(m_nGroupID, nPPID, string.Empty, string.Empty, string.Empty);
                    ...
                    else if (pphc.m_eItemType == BillingItemsType.PrePaid)
                    {
                        ...
                        thePrePaid = Pricing.Module.GetPrePaidModuleData(m_nGroupID, nItemID, string.Empty, string.Empty, string.Empty);
                        pphc.m_sPurchasedItemCode = nItemID.ToString();
                        pphc.m_sPurchasedItemName = thePrePaid.m_Title;
                ...
                case eTransactionType.Subscription:
                    {
                        ...
                        Subscription[] arrSubscriptions =
                            Utils.GetSubscriptionsDataWithCaching(new List<string>(1) { sSubCode }, m_nGroupID);
       ...
       */
        ...
        protected virtual string GetCustomDataForMPPRenewal(RenewDetails renewSubscriptionDetails, Subscription subscription, UsageModule usageModule,
                                                            ...
                                                            string deviceName = null)
        {
            ...
            sb.AppendFormat("<cc>{0}</cc>", renewSubscriptionDetails.RecurringData.CouponCode);
            ...
            if (renewSubscriptionDetails.RecurringData.Compensation != null)
            {
                sb.AppendFormat("<compensation type=\"{0}\" amount=\"{1}\" totalRenewals=\"{2}\" renewalNumber=\"{3}\" />",
                                 renewSubscriptionDetails.RecurringData.Compensation.CompensationType,
                                 renewSubscriptionDetails.RecurringData.Compensation.Amount,
                                 renewSubscriptionDetails.RecurringData.Compensation.TotalRenewals,
                                 renewSubscriptionDetails.RecurringData.Compensation.Renewals + 1);
            }
                    ...
                    else
                    {
                        ...
                        if (theReason == PriceReason.ForPurchase || bIsEntitledToPreviewModule)
                        {
                            if (bDummy || (p != null && p.m_dPrice == dPrice && p.m_oCurrency.m_sCurrencyCD3 == sCurrency))
                            {
                                ...
                                if (p.m_dPrice != 0 || bDummy)
                                {
                                    ret = HandleCellularChargeUser(m_nGroupID, sSiteGUID, dPrice, sCurrency, sUserIP,
                                                                   sCustomData, 1, nRecPeriods, sExtraParams, bDummy, bIsEntitledToPreviewModule);
        ...
        protected void UpdatePurchaseIDInExternalBillingTable(long lBillingTransactionID, long lPurchaseID)
        {
            ...
            {
                UpdatePurchaseIDInBilling(lPurchaseID, lBillingTransactionID);
            }
            else
            {
        ...
        public ChangeSubscriptionStatus ChangeSubscription(string sSiteGuid, int nOldSub, int nNewSub, string udid)
        {
            try
            {
                ...
                Subscription[] subs = Utils.GetSubscriptionsDataWithCaching(new List<int>(1) { nNewSub }, m_nGroupID);
            ...
            try
            {
                ...
                if (billResp.m_oStatus == BillingResponseStatus.Success)
                {
                    ...
                    if (Int32.TryParse(billResp.m_sRecieptCode, out nBillingTransID) && nBillingTransID > 0)
                    {
                        ...
                        int nBillingMethod = (int)ePaymentMethod.ChangeSubscription;
        ...
        private void HandleDeleteDomainUnifiedBillingCycle(int domainId, int assetId)
        {
            try
            {
            ...
            {
                log.ErrorFormat("HandleDeleteDomainUnifiedBillingCycle fail for domainId = {0} , assetID = {1}, with ex = {2}", domainId, assetId, ex);
            }
        }
            ...
            try
            {
                ...
                if (subscription != null && subscription.Type == SubscriptionType.Base)
                {
                    ...
                    if (setsIds != null && setsIds.Count > 0 && Utils.TryGetSubscriptionSets(m_nGroupID, setsIds, ref subscriptionSets))
                    {
                        ...
                        // get all household subscriptions
                        if (Utils.TryGetDomainEntitlementsFromCache(m_nGroupID, domainId, null, ref domainEntitlements))
                        {
                            // get all household with base subscription
                            if (domainEntitlements.DomainBundleEntitlements != null && domainEntitlements.DomainBundleEntitlements.EntitledSubscriptions != null
                                || domainEntitlements.DomainBundleEntitlements.EntitledSubscriptions.Count() > 0)
                            {
                                List<long> hhSubscriptionIds = domainEntitlements.DomainBundleEntitlements.EntitledSubscriptions.Select(x => long.Parse(x.Key)).ToList();
                                if (hhSubscriptionIds != null && hhSubscriptionIds.Count() > 0)
                                {
                                    //shrink addOnBase to the ones that household entiteled to 
                                    addOnBase = addOnBase.Where(x1 => hhSubscriptionIds.Any(x => x == x1)).ToList();
                                    if (addOnBase != null && addOnBase.Count() > 0)
                                    {
                                        // get all addon
                                        Subscription[] subscriptionList = Utils.GetSubscriptionsDataWithCaching(addOnBase, m_nGroupID);

                                        if (subscriptionList != null && subscriptionList.Count() > 0)
                                        {
                                            // get all dependency set for base subscription 
                                            setsIds = new List<long>();
                                                             ...
                                                             string languageCode, string deviceName, string couponCode)
        {
            ...
            return GetLicensedLinks(userId, mediaFileId, basicLink, ip, refferer, countryCode, languageCode, deviceName, couponCode, eObjectType.Media,
                                    ref fileMainStreamingCoID, ref mediaId, ref fileType, out drmId, ref fileCoGuid);

        }

        public virtual LicensedLinkResponse GetLicensedLinks(string userId, Int32 mediaFileId, string basicLink, string ip, string refferer, string countryCode,
                                                             string languageCode, string deviceName, string couponCode, ref int fileMainStreamingCoId, ref string fileType)
        {
            int mediaId = 0;
            int drmId = 0;
            string fileCoGuid = string.Empty;

            return GetLicensedLinks(userId, mediaFileId, basicLink, ip, refferer, countryCode,
                                    languageCode, deviceName, couponCode, eObjectType.Media, ref fileMainStreamingCoId,
                                    ref mediaId, ref fileType, out drmId, ref fileCoGuid);
        }
                                                             ...
                                                             ref int mediaId, ref string fileType, out int drmId, ref string fileCoGuid, long programId = 0, string npvrId = "")
        {
            ...
            try
            {
                ...
                {
                    basicLink = Utils.GetBasicLink(m_nGroupID, mediaFiles, mediaFileId, basicLink, out streamingCoID, out fileType, out drmId);
                }
            ...
            {
                response = new LicensedLinkResponse(string.Empty, string.Empty, eLicensedLinkStatus.Error.ToString(), (int)eResponseStatus.Error, eResponseStatus.Error.ToString());
                                ...
                                if (handleBillingPassed && subscriptionEndDate.HasValue)
                                {
                                    ...
                                    if (subscription.m_bIsRecurring)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            paymentGatewayResponse = Billing.Module.GetPaymentGatewayByBillingGuid(m_nGroupID, householdId, billingGuid);
            ...
            string externalMessage, int failReason, string signature)
        {
            ...
            {
                // update billing
                var billingResponse = Billing.Module.UpdatePendingTransaction(m_nGroupID, paymentGatewayId, adapterTransactionState, externalTransactionId,
                            ...
                            if (handleBillingPassed && endDate.HasValue)
                            {
                                ...
                                if (subscription.m_bIsRecurring)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        long endDateUnix = TVinciShared.DateUtils.DateTimeToUtcUnixTimestampSeconds((DateTime)endDate);
            ...
            string paymentGatewayResponseCode, int state, string paymentMethodExternalID)
        {
            ...
            {
                // call new billing method for charge adapter
                var transactionResponse = Billing.Module.RecordTransaction(m_nGroupID, userId, (int)householdId, paymentGatewayReferenceID, paymentGatewayResponseCode, productId, (int)transactionType,
            ...
            string paymentMethodExternalId)
        {
            ...
            {
                // call new billing method for charge adapter
                var response = Billing.Module.UpdateRecordedTransaction(m_nGroupID, (int)householdId, paymentGatewayReferenceID, paymentDetails, paymentMethod, paymentGatewayId,
                                        ...
                                        if (epgFieldMappings != null && epgFieldMappings.Count > 0)
                                        {
                                                ...
                                                && RecordingsDAL.IsSeriesFollowed(m_nGroupID, sereisId, seasonNum, channelId))
                                            {
                                                ...
                                                if (!isPrivateCopy)
                                                {
                                                    // record
                                                    HashSet<long> failedDomainIds;
                                                    Recording serieRecording = RecordingsManager.Instance.Record(m_nGroupID, epg.EPG_ID, channelId, startDate, endDate, epg.CRID, new List<long>(), out failedDomainIds);
                                                ...
                                                if (isPrivateCopy || isRecordingSuccess)
                                                {
                                                    DateTime distributeTime = startDate.AddMinutes(1);
                                                    eRecordingTask task = eRecordingTask.DistributeRecording;
                                                    RecordingsManager.EnqueueMessage(m_nGroupID, epg.EPG_ID, recordingId, startDate, distributeTime, task);
                                                }
                                            }
                                        }
                                    }
        ...
        private ApiObjects.Response.Status RemovePaymentMethodHouseholdInBilling(int groupId, int paymentGatewayId, string siteGuid, long householdId, int paymentMethodId, bool force)
        {
            ...
            {
                // call new billing method for charge adapter
                var billingResponse = Billing.Module.RemovePaymentMethodHouseholdPaymentGateway(m_nGroupID, paymentGatewayId, siteGuid, (int)householdId, paymentMethodId, force);
                if (billingResponse == null)
                    status = new ApiObjects.Response.Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString());
        ...
        public Recording UpdateRecord(string userId, long domainRecordingId, Recording recordingToUpdate)
        {
            ...
            try
            {
                Domain domain;
                long domainID = 0;
                ApiObjects.Response.Status validationStatus =
                    Utils.ValidateUserAndDomain(m_nGroupID, userId, ref domainID, out domain);
                ...
                {
                    log.DebugFormat(
                        "Recording is not valid for protection, recordID: {0}, DomainID: {1}, UserID: {2}, Recording: {3}",
                        domainRecordingId, domainID, userId, recording != null ? recording.ToString() : string.Empty);
                ...
                else
                {
                    ...
                    if (recordingToUpdate.IsProtected)
                    {
                        TimeShiftedTvPartnerSettings
                            accountSettings = Utils.GetTimeShiftedTvPartnerSettings(m_nGroupID);

                        // Check if protect is defined and enabled
                        if (accountSettings == null || !accountSettings.IsProtectionEnabled.HasValue)
                        {
                            log.ErrorFormat(
                                "Failed getting account protection quota percentage, DomainID: {0}, UserID: {1}, recordID: {2}",
                                domainID, userId, domainRecordingId);
                            recording.Status = new ApiObjects.Response.Status((int)eResponseStatus.Error,
                        ...
                        {
                            log.DebugFormat(
                                "Protect is not enabled on the account, DomainID: {0}, UserID: {1}, recordID: {2}",
                                domainID, userId, domainRecordingId);
                            recording.Status = new ApiObjects.Response.Status(
                                (int)eResponseStatus.AccountProtectRecordNotEnabled,
                                eResponseStatus.AccountProtectRecordNotEnabled.ToString());
                        ...
                        ApiObjects.TimeShiftedTv.DomainQuotaResponse domainQuota =
                            QuotaManager.Instance.GetDomainQuotaResponse(this.m_nGroupID, domainID);
                        if (domainQuota != null)
                        {
                        ...
                        int availableProtectionSeconds = (int)Math.Ceiling(
                            (double)((domainsQuotaInSeconds * accountSettings.ProtectionQuotaPercentage.Value) / 100));

                        // Get domain used protection minutes
                        Dictionary<long, Recording> domainProtectedRecordings =
                            Utils.GetDomainProtectedRecordings(m_nGroupID, domainID);

                        // Check protection quota before applying protection on recording
                        ApiObjects.Response.Status protectionQuotaStatus =
                            QuotaManager.Instance.CheckQuotaByTotalSeconds(
                                m_nGroupID, domainID, availableProtectionSeconds, false,
                                new List<Recording>() { recording },
                                domainProtectedRecordings != null
                                    ? domainProtectedRecordings.Values.ToList()
                                    : new List<Recording>());

                        if (protectionQuotaStatus == null || protectionQuotaStatus.Code != (int)eResponseStatus.OK ||
                            recording.Status == null || recording.Status.Code != (int)eResponseStatus.OK)
                        {
                            log.DebugFormat(
                                "Domain Exceeded Protection Quota, DomainID: {0}, UserID: {1}, recordID: {2}",
                                domainID, userId, domainRecordingId);
                            recording.Status = new ApiObjects.Response.Status(
                                (int)eResponseStatus.ExceededProtectionQuota,
                                eResponseStatus.ExceededProtectionQuota.ToString());
                        ...
                        {
                            log.ErrorFormat(
                                "Failed getting account protection period, DomainID: {0}, UserID: {1}, recordID: {2}",
                                domainID, userId, domainRecordingId);
                            recording.Status = new ApiObjects.Response.Status((int)eResponseStatus.Error,
                                eResponseStatus.Error.ToString());
            ...
            {
                StringBuilder sb = new StringBuilder("Exception at UpdateRecord. ");
                sb.Append(String.Concat("userId: ", userId));
                sb.Append(String.Concat("recordID: ", domainRecordingId));
                sb.Append(String.Concat("Ex Msg: ", ex.Message));
                sb.Append(String.Concat(", Ex Type: ", ex.GetType().Name));
        ...
        private static void UpdateRecordingSuccessed(Recording recording, long protectedUntilEpoch, long? externalViewableUntilDate = null)
        {
            ...
            if (!recording.ViewableUntilDate.HasValue ||
                (recording.ViewableUntilDate.HasValue &&
                 recording.ViewableUntilDate.Value < protectedUntilEpoch))
            {
            ...
            try
            {
                ...
                if (expiredRecordingsToSchedule != null)
                {
                    ...
                    var messagesToSend = expiredRecordingsToSchedule.Values.Select(r => new ModifiedRecordingRequest
                    {
                        ...
                        OldRecordingDuration = r.OldRecordingDuration,
                        ScheduledExpirationEpoch = r.ScheduledExpirationEpoch
                    });

                    try
                    {
                        var eventBus = EventBus.RabbitMQ.EventBusPublisherRabbitMQ.GetInstanceUsingTCMConfiguration();
                        eventBus.Publish(messagesToSend);
                    }
                    catch (Exception e)
                    {
                        log.Error($"Failed to queue ExpiredRecordingScheduledTask", e);
                    }
            ...
            try
            {
                ...
                if (epgMatch != null && epgMatch.Count > 0) // epg related to series and season
                {
                    ...
                    if (domainRecordingID == 0)
                    {
                        ...
                        Recording recording = Utils.GetRecordingByEpgId(m_nGroupID, epgId);
        ...
        public static IEnumerable<IEnumerable<T>> Chunkify<T>(IEnumerable<T> source, int size)
        {
            int count = 0;
        ...
        public string BulkRecoveryForRenewSubscriptions(DateTime endDateStartRange, DateTime endDateEndRange)
        {
            ...
            if (dt != null && dt.Rows != null && dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    {
                        var nextRenewalDate = endDate.Value.AddMinutes(renewalStartMin);
                        var endDateUnix = TVinciShared.DateUtils.DateTimeToUtcUnixTimestampSeconds(endDate.Value);
                        var data = new RenewTransactionData(groupId, siteGuid, purchaseId, billingGuid,
                                                        endDateUnix, nextRenewalDate);
        ...
        internal ApiObjects.Response.Status SuspendPaymentGatewayEntitlements(long householdId, int paymentGatewayId)
        {
            ...
            try
            {
                PaymentGateway paymentGateway = DAL.BillingDAL.GetPaymentGateway(m_nGroupID, paymentGatewayId);
                if (paymentGateway == null || paymentGateway.Status != 1 || paymentGateway.IsActive != 1)
                {
                    response = new ApiObjects.Response.Status((int)eResponseStatus.PaymentGatewayNotValid, "Payment gateway is not valid");
                    return response;
                }

                Domain household;
                ApiObjects.Response.Status domainStatus = Utils.ValidateDomain(m_nGroupID, (int)householdId, out household);
            ...
            {
                response = new ApiObjects.Response.Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString());
                log.Error(string.Format("Failed to suspend household entitlements for payment gateway. groupId = {0}, paymentGatewayId = {1}, householdId = {2}", m_nGroupID, paymentGatewayId, householdId), ex);
            }
        ...
        internal ApiObjects.Response.Status ResumePaymentGatewayEntitlements(long householdId, int paymentGatewayId)
        {
            ...
            {
                response = new ApiObjects.Response.Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString());
                log.Error(string.Format("Failed to resume household entitlements for payment gateway. groupId = {0}, paymentGatewayId = {1}, householdId = {2}", m_nGroupID, paymentGatewayId, householdId), ex);
            }
        ...
        internal bool SubscriptionEnds(string siteGuid, long householdId, long purchaseId, long endDate, bool isReminder)
        {
            ...
            {
                // Illegal purchase ID  
                log.ErrorFormat("Subscription ends - illegal purchaseId {0}", purchaseId);
                return true;
            }
            ...
            {
                // Illegal purchase ID  
                log.ErrorFormat("Subscription ends - failed getting data for purchaseId {0}", purchaseId);
                return true;
            }

            DateTime endDateFromRow = ODBCWrapper.Utils.ExtractDateTime(dataRow, "end_date");
            string subscriptionCode = ODBCWrapper.Utils.ExtractString(dataRow, "subscription_code");
            string billingGuid = ODBCWrapper.Utils.ExtractString(dataRow, "billing_guid");
            string customData = ODBCWrapper.Utils.ExtractString(dataRow, "customdata");
            string couponCode = ODBCWrapper.Utils.ExtractString(dataRow, "coupon_code");
            string country = ODBCWrapper.Utils.ExtractString(dataRow, "COUNTRY_CODE");
            string currency = ODBCWrapper.Utils.ExtractString(dataRow, "CURRENCY_CD");
