...
namespace Core.Users
{
    public abstract class BaseUsers
    {
        ...
        public abstract GenericResponse<ResponseStatus> RenewPassword(string sUN, string sPass, int nGroupID);
        ...
        public abstract GenericResponse<UserResponseObject> ChangeUserPassword(string sUN, string sOldPass, string sPass, Int32 nGroupID);
        ...
        public virtual ApiObjects.Response.Status DeleteItemFromUsersList(int itemId, ListType listType, ListItemType itemType, string userId, int groupId)
        {
            ApiObjects.Response.Status response = new ApiObjects.Response.Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString());

            try
            {
                if (itemId == 0 || string.IsNullOrEmpty(userId) || listType == ListType.All || itemType == ListItemType.All)
                    return response;
        ...
        public UsersListItemResponse AddItemToUsersList(int itemId, ListType listType, ListItemType itemType, int order, string userId, int groupId)
        {
            ...
            {
                int parsedUserId = 0;
                if (!int.TryParse(userId, out parsedUserId) || itemId == 0 || string.IsNullOrEmpty(userId) || listType == ListType.All || itemType == ListItemType.All)
                    return response;
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    response.Item = new Item()
                    {
                        ...
                        ItemType = (ListItemType)ODBCWrapper.Utils.GetIntSafeVal(dt.Rows[0]["item_type"]),
        ...
        public UsersListItemResponse GetItemFromUsersList(int itemId, ListType listType, ListItemType itemType, string userId, int groupId)
        {
            UsersListItemResponse response = new UsersListItemResponse()
            {
                Status = new ApiObjects.Response.Status((int)eResponseStatus.Error, eResponseStatus.Error.ToString())
