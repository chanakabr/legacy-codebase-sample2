    ...
    public class PricingMappings
    {
        public static void RegisterMappings(MapperConfigurationExpression cfg)
        {
            ...
            cfg.CreateMap<CouponsGroup, KalturaCouponsGroup>()
               .ForMember(dest => dest.Descriptions, opt => opt.ResolveUsing(src => src.m_sDescription))
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dEndDate)))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_sGroupCode))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sGroupName))
               .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dStartDate)))
               .ForMember(dest => dest.MaxUsesNumber, opt => opt.ResolveUsing(src => src.m_nMaxUseCountForCoupon))
               .ForMember(dest => dest.MaxUsesNumberOnRenewableSub, opt => opt.ResolveUsing(src => src.m_nMaxRecurringUsesCountForCoupon))
               .ForMember(dest => dest.CouponGroupType, opt => opt.ResolveUsing(src => ConvertCouponGroupType(src.couponGroupType)))
               .ForMember(dest => dest.MaxHouseholdUses, opt => opt.ResolveUsing(src => src.maxDomainUses))
               .ForMember(dest => dest.DiscountCode, opt => opt.ResolveUsing(src => long.Parse(src.m_sDiscountCode)))
               .ForMember(dest => dest.DiscountId, opt => opt.ResolveUsing(src => long.Parse(src.m_sDiscountCode)))
               ;

            cfg.CreateMap<SubscriptionCouponGroup, KalturaCouponsGroup>()
               .ForMember(dest => dest.Descriptions, opt => opt.ResolveUsing(src => src.m_sDescription))
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => (!src.endDate.HasValue || src.m_dEndDate < src.endDate.Value) ?
                   SerializationUtils.ConvertToUnixTimestamp(src.m_dEndDate) : SerializationUtils.ConvertToUnixTimestamp(src.endDate.Value)))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_sGroupCode))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sGroupName))
               .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(src => (!src.endDate.HasValue || src.m_dEndDate < src.endDate.Value) ?
            ...
            cfg.CreateMap<Price, KalturaPrice>()
               ...
               .ForMember(dest => dest.CountryId, opt => opt.ResolveUsing(src => src.countryId != 0 ? (long?)src.countryId : null))
               ;

            // PriceCode
            cfg.CreateMap<PriceCode, KalturaPriceDetails>()
               .ForMember(dest => dest.Descriptions, opt => opt.ResolveUsing(src => src.m_sDescription))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_nObjectID))
               .ForMember(dest => dest.name, opt => opt.ResolveUsing(src => src.m_sCode))
               .ForMember(dest => dest.Price, opt => opt.ResolveUsing(src => src.m_oPrise));

            // PriceCode
            cfg.CreateMap<KalturaPriceDetails, PriceCode>()
                .ForMember(dest => dest.m_sDescription, opt => opt.ResolveUsing(src => src.Descriptions != null && src.Descriptions.Count > 0 ? src.Descriptions[0] : null)) // TODO: ???
               .ForMember(dest => dest.m_nObjectID, opt => opt.ResolveUsing(src => src.Id))
               .ForMember(dest => dest.m_sCode, opt => opt.ResolveUsing(src => src.name))
               .ForMember(dest => dest.m_oPrise, opt => opt.ResolveUsing(src => src.Price));

            // DiscountModule
            cfg.CreateMap<DiscountModule, KalturaDiscountModule>()
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dEndDate)))
               .ForMember(dest => dest.Percent, opt => opt.ResolveUsing(src => src.m_dPercent))
               .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dStartDate)));

            // UsageModule
            cfg.CreateMap<UsageModule, KalturaUsageModule>()
               .ForMember(dest => dest.CouponId, opt => opt.ResolveUsing(src => src.m_coupon_id))
               .ForMember(dest => dest.FullLifeCycle, opt => opt.ResolveUsing(src => src.m_tsMaxUsageModuleLifeCycle))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_nObjectID))
               .ForMember(dest => dest.IsOfflinePlayback, opt => opt.ResolveUsing(src => src.m_bIsOfflinePlayBack))
               .ForMember(dest => dest.IsWaiverEnabled, opt => opt.ResolveUsing(src => src.m_bWaiver))
               .ForMember(dest => dest.MaxViewsNumber, opt => opt.ResolveUsing(src => src.m_nMaxNumberOfViews))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sVirtualName))
               .ForMember(dest => dest.ViewLifeCycle, opt => opt.ResolveUsing(src => src.m_tsViewLifeCycle))
               .ForMember(dest => dest.WaiverPeriod, opt => opt.ResolveUsing(src => src.m_nWaiverPeriod));
            ...
            cfg.CreateMap<PreviewModule, KalturaPreviewModule>()
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_nID))
               .ForMember(dest => dest.LifeCycle, opt => opt.ResolveUsing(src => src.m_tsFullLifeCycle))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sName))
               .ForMember(dest => dest.NonRenewablePeriod, opt => opt.ResolveUsing(src => src.m_tsNonRenewPeriod));
            ...
            cfg.CreateMap<NpvrServiceObject, KalturaNpvrPremiumService>()
               .ForMember(dest => dest.QuotaInMinutes, opt => opt.ResolveUsing(src => src.Quota))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.ID))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.Name));
            // LanguageContainer to TranslationContainer
            cfg.CreateMap<LanguageContainer, KalturaTranslationToken>()
               .ForMember(dest => dest.Language, opt => opt.ResolveUsing(src => src.LanguageCode))
            ...
            cfg.CreateMap<BundleCodeContainer, KalturaBaseChannel>()
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_sCode));

            // BundleCodeContainer to SlimChannel
            cfg.CreateMap<SubscriptionsPricesContainer, KalturaSubscriptionPrice>()
               .ForMember(dest => dest.ProductId, opt => opt.ResolveUsing(src => src.m_sSubscriptionCode))
               .ForMember(dest => dest.PurchaseStatus, opt => opt.ResolveUsing(src => ConvertPriceReasonToPurchaseStatus(src.m_PriceReason)))
               .ForMember(dest => dest.Price, opt => opt.ResolveUsing(src => src.m_oPrice))
               .ForMember(dest => dest.ProductId, opt => opt.ResolveUsing(src => src.m_sSubscriptionCode))
               .ForMember(dest => dest.ProductType, opt => opt.ResolveUsing(src => KalturaTransactionType.subscription))
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => src.endDate.HasValue ? src.endDate.Value : 0))
               ;


            // Subscription
            cfg.CreateMap<Subscription, KalturaSubscription>()
               .ForMember(dest => dest.IsInfiniteRenewal, opt => opt.ResolveUsing(src => src.m_bIsInfiniteRecurring))
               .ForMember(dest => dest.IsRenewable, opt => opt.ResolveUsing(src => src.m_bIsRecurring))
               .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dStartDate)))
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.m_dEndDate)))
               .ForMember(dest => dest.MediaId, opt => opt.ResolveUsing(src => src.m_fictivicMediaID))
               //.ForMember(dest => dest.PremiumServices, opt => opt.ResolveUsing(src => src.m_lServices))
               .ForMember(dest => dest.PremiumServices, opt => opt.ResolveUsing(src => ConvertServices(src.m_lServices)))
               .ForMember(dest => dest.PricePlans, opt => opt.ResolveUsing(src => src.m_MultiSubscriptionUsageModule))
               .ForMember(dest => dest.HouseholdLimitationsId, opt => opt.ResolveUsing(src => src.m_nDomainLimitationModule))
               .ForMember(dest => dest.RenewalsNumber, opt => opt.ResolveUsing(src => src.m_nNumberOfRecPeriods))
               .ForMember(dest => dest.CouponsGroup, opt => opt.ResolveUsing(src => src.m_oCouponsGroup))
               .ForMember(dest => dest.DiscountModule, opt => opt.ResolveUsing(src => src.m_oDiscountModule))
               .ForMember(dest => dest.PreviewModule, opt => opt.ResolveUsing(src => src.m_oPreviewModule))
               .ForMember(dest => dest.Price, opt => opt.ResolveUsing(src => src.m_oSubscriptionPriceCode))
               .ForMember(dest => dest.MaxViewsNumber, opt => opt.ResolveUsing(src => src.m_oSubscriptionUsageModule.m_nMaxNumberOfViews))
               .ForMember(dest => dest.ViewLifeCycle, opt => opt.ResolveUsing(src => src.m_oSubscriptionUsageModule.m_tsViewLifeCycle))
               .ForMember(dest => dest.WaiverPeriod, opt => opt.ResolveUsing(src => src.m_oSubscriptionUsageModule.m_nWaiverPeriod))
               .ForMember(dest => dest.IsWaiverEnabled, opt => opt.ResolveUsing(src => src.m_oSubscriptionUsageModule.m_bWaiver))
               .ForMember(dest => dest.ProrityInOrder, opt => opt.ResolveUsing(src => src.m_Priority))
               .ForMember(dest => dest.ProductCode, opt => opt.ResolveUsing(src => src.m_ProductCode))
               .ForMember(dest => dest.Channels, opt => opt.ResolveUsing(src => src.m_sCodes))
               .ForMember(dest => dest.Description, opt => opt.ResolveUsing(src => new KalturaMultilingualString(src.m_sDescription)))
               .ForMember(dest => dest.Descriptions, opt => opt.ResolveUsing(src => src.m_sDescription))
               .ForMember(dest => dest.FileTypes, opt => opt.ResolveUsing(src => src.m_sFileTypes))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => new KalturaMultilingualString(src.m_sName)))
               .ForMember(dest => dest.Names, opt => opt.ResolveUsing(src => src.m_sName))
               .ForMember(dest => dest.GracePeriodMinutes, opt => opt.ResolveUsing(src => src.m_GracePeriodMinutes))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_SubscriptionCode))
               .ForMember(dest => dest.UserTypes, opt => opt.ResolveUsing(src => src.m_UserTypes))
               ...
               .ForMember(dest => dest.ExternalId, opt => opt.ResolveUsing(src => src.m_ProductCode))
               .ForMember(dest => dest.IsCancellationBlocked, opt => opt.ResolveUsing(src => src.BlockCancellation))
               .ForMember(dest => dest.PricePlanIds, opt => opt.ResolveUsing(src =>
            ...
            cfg.CreateMap<UsageModule, KalturaPricePlan>()
               ...
               .ForMember(dest => dest.FullLifeCycle, opt => opt.ResolveUsing(src => src.m_tsMaxUsageModuleLifeCycle))
               .ForMember(dest => dest.IsOfflinePlayback, opt => opt.ResolveUsing(src => src.m_bIsOfflinePlayBack))
               .ForMember(dest => dest.IsRenewable, opt => opt.ResolveUsing(src => src.m_is_renew))
               .ForMember(dest => dest.IsWaiverEnabled, opt => opt.ResolveUsing(src => src.m_bWaiver))
               .ForMember(dest => dest.MaxViewsNumber, opt => opt.ResolveUsing(src => src.m_nMaxNumberOfViews))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sVirtualName))
               .ForMember(dest => dest.PriceId, opt => opt.ResolveUsing(src => src.m_pricing_id))
               .ForMember(dest => dest.RenewalsNumber, opt => opt.ResolveUsing(src => src.m_num_of_rec_periods))
               .ForMember(dest => dest.ViewLifeCycle, opt => opt.ResolveUsing(src => src.m_tsViewLifeCycle))
               .ForMember(dest => dest.WaiverPeriod, opt => opt.ResolveUsing(src => src.m_nWaiverPeriod))
               .ForMember(dest => dest.DiscountId, opt => opt.ResolveUsing(src => src.m_ext_discount_id))
               .ForMember(dest => dest.PriceDetailsId, opt => opt.ResolveUsing(src => src.m_pricing_id))
               ;

            // ItemPriceContainer to PPVItemPriceDetails
            cfg.CreateMap<ItemPriceContainer, KalturaPPVItemPriceDetails>()
               .ForMember(dest => dest.CollectionId, opt => opt.ResolveUsing(src => src.m_relevantCol))
               .ForMember(dest => dest.EndDate, opt => opt.ResolveUsing(src => src.m_dtEndDate.HasValue ? SerializationUtils.ConvertToUnixTimestamp(src.m_dtEndDate.Value) : 0))
               .ForMember(dest => dest.DiscountEndDate, opt => opt.ResolveUsing(src => src.m_dtDiscountEndDate.HasValue ? SerializationUtils.ConvertToUnixTimestamp(src.m_dtDiscountEndDate.Value) : 0))
               .ForMember(dest => dest.FirstDeviceName, opt => opt.ResolveUsing(src => src.m_sFirstDeviceNameFound))
               .ForMember(dest => dest.FullPrice, opt => opt.ResolveUsing(src => src.m_oFullPrice))
               .ForMember(dest => dest.IsInCancelationPeriod, opt => opt.ResolveUsing(src => src.m_bCancelWindow))
               .ForMember(dest => dest.IsSubscriptionOnly, opt => opt.ResolveUsing(src => src.m_bSubscriptionOnly))
               .ForMember(dest => dest.PPVDescriptions, opt => opt.ResolveUsing(src => src.m_oPPVDescription))
               .ForMember(dest => dest.PPVModuleId, opt => opt.ResolveUsing(src => src.m_sPPVModuleCode))
               .ForMember(dest => dest.PrePaidId, opt => opt.ResolveUsing(src => src.m_relevantPP))
               .ForMember(dest => dest.Price, opt => opt.ResolveUsing(src => src.m_oPrice))
               .ForMember(dest => dest.PurchasedMediaFileId, opt => opt.ResolveUsing(src => src.m_lPurchasedMediaFileID))
               .ForMember(dest => dest.PurchaseStatus, opt => opt.ResolveUsing(src => ConvertPriceReasonToPurchaseStatus(src.m_PriceReason)))
               .ForMember(dest => dest.PurchaseUserId, opt => opt.ResolveUsing(src => src.m_sPurchasedBySiteGuid))
               .ForMember(dest => dest.RelatedMediaFileIds, opt => opt.ResolveUsing(src => src.m_lRelatedMediaFileIDs))
               .ForMember(dest => dest.StartDate, opt => opt.ResolveUsing(src => src.m_dtStartDate.HasValue ? SerializationUtils.ConvertToUnixTimestamp(src.m_dtStartDate.Value) : 0))
               .ForMember(dest => dest.SubscriptionId, opt => opt.ResolveUsing(src => src.m_relevantSub.m_sObjectCode))
               .ForMember(dest => dest.ProductCode, opt => opt.ResolveUsing(src => src.m_sProductCode));

            // ItemPriceContainer to PPVItemPriceDetails
            cfg.CreateMap<MediaFileItemPricesContainer, KalturaItemPrice>()
               .ForMember(dest => dest.ProductId, opt => opt.ResolveUsing(src => src.m_sProductCode))
               .ForMember(dest => dest.FileId, opt => opt.ResolveUsing(src => src.m_nMediaFileID))
               .ForMember(dest => dest.PPVPriceDetails, opt => opt.ResolveUsing(src => src.m_oItemPrices));

            // CouponData to CouponDetails
            cfg.CreateMap<CouponData, KalturaCoupon>()
               .ForMember(dest => dest.CouponsGroup, opt => opt.ResolveUsing(src => src.m_oCouponGroup))
               .ForMember(dest => dest.Status, opt => opt.ResolveUsing(src => ConvertCouponStatus(src.m_CouponStatus)))
               .ForMember(dest => dest.LeftUses, opt => opt.ResolveUsing(src => src.leftUses))
               .ForMember(dest => dest.TotalUses, opt => opt.ResolveUsing(src => src.totalUses))
               ;
            ...
            cfg.CreateMap<PPVModule, KalturaPpv>()
               ...
               .ForMember(dest => dest.Descriptions, opt => opt.ResolveUsing(src => src.m_sDescription))
               .ForMember(dest => dest.DiscountModule, opt => opt.ResolveUsing(src => src.m_oDiscountModule))
               .ForMember(dest => dest.FileTypes, opt => opt.ResolveUsing(src => src.m_relatedFileTypes))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_sObjectCode))
               .ForMember(dest => dest.IsSubscriptionOnly, opt => opt.ResolveUsing(src => src.m_bSubscriptionOnly))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.m_sObjectVirtualName))
               .ForMember(dest => dest.Price, opt => opt.ResolveUsing(src => src.m_oPriceCode))
               .ForMember(dest => dest.ProductCode, opt => opt.ResolveUsing(src => src.m_Product_Code))
               .ForMember(dest => dest.UsageModule, opt => opt.ResolveUsing(src => src.m_oUsageModule))
               .ForMember(dest => dest.FirstDeviceLimitation, opt => opt.ResolveUsing(src => src.m_bFirstDeviceLimitation));
            ...
            cfg.CreateMap<SwitchSet, KalturaSubscriptionSwitchSet>()
                .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.Id))
                .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.Name))
                .ForMember(dest => dest.SubscriptionIds, opt => opt.ResolveUsing(src => src.SubscriptionIds != null ? string.Join(",", src.SubscriptionIds) : string.Empty))
            ...
            cfg.CreateMap<DependencySet, KalturaSubscriptionDependencySet>()
                .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.Id))
                .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => src.Name))
                .ForMember(dest => dest.BaseSubscriptionId, opt => opt.ResolveUsing(src => src.BaseSubscriptionId))
                .ForMember(dest => dest.SubscriptionIds, opt => opt.ResolveUsing(src => src.AddOnIds != null ? string.Join(",", src.AddOnIds) : string.Empty))
                .ForMember(dest => dest.Type, opt => opt.ResolveUsing(src => ConvertSetType(src.Type)));
            ;
            ...
            cfg.CreateMap<PriceDetails, KalturaPriceDetails>()
                .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.Id))
                .ForMember(dest => dest.name, opt => opt.ResolveUsing(src => src.Name))
                .ForMember(dest => dest.MultiCurrencyPrice, opt => opt.ResolveUsing(src => src.Prices))
                ;

            // KalturaPricePlan
            cfg.CreateMap<KalturaPricePlan, UsageModule>()
               .ForMember(dest => dest.m_nObjectID, opt => opt.ResolveUsing(src => src.Id))
               .ForMember(dest => dest.m_coupon_id, opt => opt.ResolveUsing(src => src.CouponId))
               .ForMember(dest => dest.m_tsMaxUsageModuleLifeCycle, opt => opt.ResolveUsing(src => src.FullLifeCycle))
               .ForMember(dest => dest.m_bIsOfflinePlayBack, opt => opt.ResolveUsing(src => src.IsOfflinePlayback))
               .ForMember(dest => dest.m_is_renew, opt => opt.ResolveUsing(src => src.IsRenewable))
               .ForMember(dest => dest.m_bWaiver, opt => opt.ResolveUsing(src => src.IsWaiverEnabled))
               .ForMember(dest => dest.m_nMaxNumberOfViews, opt => opt.ResolveUsing(src => src.MaxViewsNumber))
               .ForMember(dest => dest.m_sVirtualName, opt => opt.ResolveUsing(src => src.Name))
               .ForMember(dest => dest.m_num_of_rec_periods, opt => opt.ResolveUsing(src => src.RenewalsNumber))
               .ForMember(dest => dest.m_tsViewLifeCycle, opt => opt.ResolveUsing(src => src.ViewLifeCycle))
               .ForMember(dest => dest.m_nWaiverPeriod, opt => opt.ResolveUsing(src => src.WaiverPeriod))
               .ForMember(dest => dest.m_ext_discount_id, opt => opt.ResolveUsing(src => src.DiscountId))
               .ForMember(dest => dest.m_pricing_id, opt => opt.ResolveUsing(src => src.PriceDetailsId.HasValue ? src.PriceDetailsId : src.PriceId))
               ;
            ...
            cfg.CreateMap<Collection, KalturaCollection>()
               ...
               .ForMember(dest => dest.Description, opt => opt.ResolveUsing(src => new KalturaMultilingualString(src.m_sDescription)))
               .ForMember(dest => dest.Name, opt => opt.ResolveUsing(src => new KalturaMultilingualString(src.m_sName)))
               .ForMember(dest => dest.Id, opt => opt.ResolveUsing(src => src.m_CollectionCode))
               .ForMember(dest => dest.ExternalId, opt => opt.ResolveUsing(src => src.m_ProductCode))
               .ForMember(dest => dest.UsageModule, opt => opt.ResolveUsing(src => src.m_oCollectionUsageModule))
               .ForMember(dest => dest.PriceDetailsId, opt => opt.ResolveUsing(src => src.m_oCollectionPriceCode != null ? src.m_oCollectionPriceCode.m_nObjectID : 0))
               .ForMember(dest => dest.ProductCodes, opt => opt.ResolveUsing(src => ConvertProductCodes(src.ExternalProductCodes)))
               .ForMember(dest => dest.CouponGroups, opt => opt.ResolveUsing(src => ConvertCouponsGroup(src.CouponsGroups)))
               ;
            ...
            cfg.CreateMap<DiscountDetails, KalturaDiscountDetails>()
               ...
               .ForMember(dest => dest.EndtDate, opt => opt.ResolveUsing(src => SerializationUtils.ConvertToUnixTimestamp(src.EndDate)))
               .ForMember(dest => dest.MultiCurrencyDiscount, opt => opt.ResolveUsing(src => src.MultiCurrencyDiscounts))
               ;
            ...
            cfg.CreateMap<Discount, KalturaDiscount>()
               ...
               .ForMember(dest => dest.CurrencySign, opt => opt.ResolveUsing(src => src.m_oCurrency.m_sCurrencySign))
               .ForMember(dest => dest.CountryId, opt => opt.ResolveUsing(src => src.countryId != 0 ? (long?)src.countryId : null))
               .ForMember(dest => dest.Percentage, opt => opt.ResolveUsing(src => src.Percentage))
               ;
        }
