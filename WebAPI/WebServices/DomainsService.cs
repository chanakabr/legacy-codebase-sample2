...
{
    public class DomainsService : System.Web.Services.WebService, IDomainsService
    {
        [WebMethod]
        ...
        public DeviceResponse AddDevice(string sWSUserName, string sWSPassword, int nDomainID, string udid, string deviceName, int deviceBrandID)
        {
            ...
            if (nGroupID != 0)
            {
                response = Core.Domains.Module.AddDevice(nGroupID, nDomainID, udid, deviceName, deviceBrandID, string.Empty);
            }
            else
            {
                if (nGroupID == 0)
        ...
        public ApiObjects.Response.Status SetDeviceInfo(string sWSUserName, string sWSPassword, string sDeviceUDID, string sDeviceName)
        {
            ...
            Int32 nGroupID = Core.Users.Utils.GetGroupID(sWSUserName, sWSPassword, "SetDeviceInfo", ref t);
            if (nGroupID != 0)
            {
            ...
            string sNetworkID, string sNetworkName, string sNetworkDesc, bool bIsActive)
        {
            ...
            Int32 nGroupID = Core.Users.Utils.GetGroupID(sWSUsername, sWSPassword, "UpdateDomainHomeNetwork", ref t);
        ...
        {
            Int32 groupId = Core.Users.Utils.GetDomainGroupID(sWSUserName, sWSPassword);
            if (groupId != 0)
            {
        ...
        public ApiObjects.Response.Status RemoveDomainById(string sWSUserName, string sWSPassword, int nDomainID, bool purge)
        {
            ...
            if (nGroupID != 0)
            {
                ...
                status = Core.Users.Utils.ConvertDomainResponseStatusToResponseObject(domainResponseStatus);
            }
            else
            {
                if (nGroupID == 0)
        ...
        public DeviceResponse SubmitAddDeviceToDomain(string sWSUserName, string sWSPassword, int domainID, string userID, string deviceUdid, string deviceName, int brandID)
        {
            ...
            if (nGroupID != 0)
            {
                response = Core.Domains.Module.SubmitAddDeviceToDomain(nGroupID, domainID, userID, deviceUdid, deviceName, brandID, string.Empty);
            }
            else
            {
                if (nGroupID == 0)
