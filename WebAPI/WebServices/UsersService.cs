    ...
    public class UsersService : System.Web.Services.WebService, IUsersService
    {
        ...
        public virtual UserResponseObject KalturaSignOut(string sWSUserName, string sWSPassword, string sSiteGUID, string sessionID, string sIP, string deviceID, bool bPreventDoubleLogins, List<ApiObjects.KeyValuePair> keyValueList)
        {
            try
            {
                ...
                Core.Users.Utils.GetGroupID(sWSUserName, sWSPassword, "SignIn", ref kUser);
        ...
        public virtual UserResponseObject AddNewUser(string sWSUserName, string sWSPassword, UserBasicData oBasicData, UserDynamicData sDynamicData, string sPassword, string sAffiliateCode)
        {
            ...
            if (Core.Users.Utils.IsGroupIDContainedInConfig(nGroupID))
            {
                // old Core to PS flow

                Core.Users.Utils.GetGroupID(sWSUserName, sWSPassword);
                if (nGroupID != 0)
                {
        ...
        public virtual UserResponseObject AddNewKalturaUser(string sWSUserName, string sWSPassword, UserBasicData oBasicData, UserDynamicData sDynamicData, string sPassword, string sAffiliateCode, List<ApiObjects.KeyValuePair> keyValueList)
        {
            ...
            // get group ID + user type
            int nGroupID = Core.Users.Utils.GetGroupID(sWSUserName, sWSPassword, "SignIn", ref kUser);
            if (nGroupID != 0 && kUser != null)
            {
        ...
        */
        ...
        public virtual Core.Users.Country[] GetCountryList(string sWSUserName, string sWSPassword)
        {
            ...
            if (nGroupID != 0)
            {
                return Core.Users.Utils.GetCountryList();
            }
            else
            {
                HttpContext.Current.Response.StatusCode = 404;
        ...
        public virtual State[] GetStateList(string sWSUserName, string sWSPassword, Int32 nCountryID)
        {
            ...
            if (nGroupID != 0)
            {
                return Core.Users.Utils.GetStateList(nCountryID);
            }
            else
            {
                HttpContext.Current.Response.StatusCode = 404;
        ...
        public virtual UserResponse LogIn(string sWSUserName, string sWSPassword, string userName, string password, string sessionID, string sIP, string deviceID, bool bPreventDoubleLogins, List<KeyValuePair> keyValueList)
        {
            ...
            if (nGroupID != 0)
            {
                ...
                if (response.user != null)
                {
                    ...
                    response.resp = Core.Users.Utils.ConvertResponseStatusToResponseObject(response.user.m_RespStatus);
                    int userID;
                    if (response.resp.Code == (int)eResponseStatus.OK && int.TryParse(response.user.m_user.m_sSiteGUID, out userID) && userID > 0)
                    {
                        Core.Users.Utils.AddInitiateNotificationActionToQueue(nGroupID, eUserMessageAction.Login, userID, deviceID);
                    }
                    else
                    {
        ...
        public virtual ApiObjects.Response.Status DeleteUser(string sWSUserName, string sWSPassword, int userId)
        {
            ...
            {
                Core.Users.Utils.AddInitiateNotificationActionToQueue(nGroupID, eUserMessageAction.DeleteUser, userId, string.Empty);
            }
            else
        ...
        public void AddInitiateNotificationAction(string sWSUserName, string sWSPassword, eUserMessageAction userAction, int userId, string udid, string pushToken = "")
        {
            ...
            if (nGroupID != 0)
            {
                Core.Users.Utils.AddInitiateNotificationActionToQueue(nGroupID, eUserMessageAction.AnonymousPushRegistration, userId, udid, pushToken);
            }
            else
            {
                HttpContext.Current.Response.StatusCode = 404;
