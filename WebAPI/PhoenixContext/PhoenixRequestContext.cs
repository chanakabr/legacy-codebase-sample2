...
namespace Phoenix.Context
{
    ...
    public class PhoenixRequestContext
    {
        ...
        {
            Ks = GetOrSetFromHttpContext(RequestContext.REQUEST_KS, Ks);
            GroupId = GetOrSetStuctFromHttpContext(RequestContext.REQUEST_GROUP_ID, GroupId);
            UserId = GetOrSetStuctFromHttpContext(RequestContext.REQUEST_USER_ID, UserId);
            UserIpAdress = GetOrSetFromHttpContext(RequestContext.USER_IP, UserIpAdress);
            RouteData.Service = GetOrSetFromHttpContext(RequestContext.REQUEST_SERVICE, RouteData.Service);
            RouteData.Action = GetOrSetFromHttpContext(RequestContext.REQUEST_ACTION, RouteData.Action);
            RouteData.PathData = GetOrSetFromHttpContext(RequestContext.REQUEST_PATH_DATA, RouteData.PathData);
            Format = GetOrSetFromHttpContext(RequestContext.REQUEST_FORMAT, Format);
            Currency = GetOrSetFromHttpContext(RequestContext.REQUEST_CURRENCY, Currency);
            AbortOnError = GetOrSetStuctFromHttpContext(RequestContext.MULTI_REQUEST_GLOBAL_ABORT_ON_ERROR, AbortOnError);
            ActionParams = GetOrSetFromHttpContext(RequestContext.REQUEST_METHOD_PARAMETERS, ActionParams);
            ResponseProfile = GetOrSetFromHttpContext(RequestContext.REQUEST_RESPONSE_PROFILE, ResponseProfile);
            RequestContentType = GetOrSetFromHttpContext(RequestContext.REQUEST_SERVE_CONTENT_TYPE, RequestContentType);
            RequestDate = GetOrSetStuctFromHttpContext(RequestContext.REQUEST_TIME, RequestDate);
            RequestType = GetOrSetStuctFromHttpContext(RequestContext.REQUEST_TYPE, RequestType);
            RequestVersion = GetOrSetFromHttpContext(RequestContext.REQUEST_VERSION, RequestVersion);
        }
