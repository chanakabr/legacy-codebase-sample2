...
namespace WebAPI.Clients
{
    public class BillingClient : BaseClient
    {
        ...
        internal KalturaPaymentGatewayConfiguration GetPaymentGatewayConfiguration(int groupId, string paymanetGatewayExternalId)
        {
            ...
            try
            {
                using (KMonitor km = new KMonitor(Events.eEvent.EVENT_WS))
                {
                    response = Billing.GetPaymentGatewayConfiguration(group.BillingCredentials.Username, group.BillingCredentials.Password, paymanetGatewayExternalId);
                }
            }
            catch (Exception ex)
            {
