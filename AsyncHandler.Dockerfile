...
RUN dotnet publish -c Release "./Phoenix.AsyncHandler/Phoenix.AsyncHandler.csproj" -o /src/published
...
# a non-root user, kaltura, will run this docker - so we won't give permissions accidently
USER kaltura
ENTRYPOINT [ "sh", "-c", "dotnet ./Phoenix.AsyncHandler.dll --urls http://0.0.0.0:${PORT}" ]
