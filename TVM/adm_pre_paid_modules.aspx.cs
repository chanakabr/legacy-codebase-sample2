...
public partial class adm_pre_paid_modules : System.Web.UI.Page
{
    ...
    protected void FillTheTableEditor(ref DBTableWebEditor theTable, string sOrderBy)
    {
        ...
        theTable += "select ppm.is_active,ppm.id as id,ppm.NAME as 'Code',ppm.ID as 'Pre Paid Module ID',p1.Code as 'Price', p2.Code as 'Credit Value', ppm.status,lcs.description as 'State' from price_codes p1, price_codes p2, pre_paid_modules ppm,lu_content_status lcs where lcs.id=ppm.status and ppm.status<>2 and ";
        theTable += "ppm.Price_Code = p1.id and ppm.Value_Price_Code = p2.id and ";
        theTable += ODBCWrapper.Parameter.NEW_PARAM("ppm.group_id", "=", nGroupID);
        if (sOrderBy != "")
        {
            theTable += " order by ";
            theTable += sOrderBy;
        ...
        theTable.AddActivationField("pre_paid_modules");
        theTable.AddHiddenField("is_active");
        string sGroupLang = GetMainLang();
        /*
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.EDIT))
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_ppv_module_descriptions.aspx", "Descriptions", "");
            string sQuery = "select description as 'txt' from ppv_descriptions where group_id=" + LoginManager.GetLoginGroupID().ToString() + " and status=1 and is_active=1 and language_code3='" + sGroupLang + "' and ppv_module_id=";
            linkColumn1.SetColumnTextDynamicQuery(sQuery);
            linkColumn1.AddQueryStringValue("ppv_module_id", "field=id");
            theTable.AddLinkColumn(linkColumn1);
        }
        */
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.EDIT))
        {
            DataTableLinkColumn linkColumn1 = new DataTableLinkColumn("adm_pre_paid_modules_new.aspx", "Edit", "");
            linkColumn1.AddQueryStringValue("ppv_module_id", "field=id");
            theTable.AddLinkColumn(linkColumn1);
        }
        ...
        if (LoginManager.IsActionPermittedOnPage(LoginManager.PAGE_PERMISION_TYPE.REMOVE))
        {
            ...
            linkColumn.AddQueryStringValue("table", "pre_paid_modules");
    ...
    {
        Response.Write(PageUtils.GetPreHeader() + ": Pre Paid Modules");
    }
}