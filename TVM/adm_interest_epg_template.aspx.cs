...
public partial class adm_interest_epg_template : System.Web.UI.Page
{
    ...
    private string GetCurrentValue(string fieldName, string tableName, int templateId, string connectionString)
    {
        string sRet = "";
        ODBCWrapper.DataSetSelectQuery selectQuery = new ODBCWrapper.DataSetSelectQuery();
        selectQuery += "select " + fieldName + " from " + tableName + " where ";
        ...
        if (selectQuery.Execute("query", true) != null)
        {
            ...
            if (nCount > 0)
            {
                object oRet = selectQuery.Table("query").DefaultView[0].Row[fieldName];
