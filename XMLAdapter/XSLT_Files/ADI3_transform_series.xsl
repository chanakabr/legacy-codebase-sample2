...
<xsl:output method="xml" omit-xml-declaration="yes" indent="yes"/>
  ...
  <xsl:template match="/">
    <feed broadcasterName="whitelabel">
      <export>
        ...
        <xsl:if test="translate($OfferKind, $uppercase, $smallcase) = 'series'">
          <xsl:apply-templates select="//*[local-name()='Title'][@type = 'SeriesTitle']"/>
        </xsl:if>
        <xsl:apply-templates select="//*[local-name()='ContentGroup'][@type = 'CastContentGroup']"/>
      </export>
    </feed>
  </xsl:template>

  <xsl:template match="//*[local-name()='ContentGroup'][@type = 'CastContentGroup']">
    <xsl:element name="media">
      <xsl:attribute name="is_active">true</xsl:attribute>
      <xsl:attribute name="co_guid">
        <xsl:value-of select="*[local-name()='TitleRef']/@uriId"/>
      </xsl:attribute>
      <xsl:attribute name="action">insert</xsl:attribute>
      <xsl:call-template name="build_cast_basic_data"/>
    </xsl:element>
  </xsl:template>

  <xsl:template name="build_cast_basic_data">
    <xsl:element name="basic">
      <xsl:variable name="CasturiId" select="*[local-name()='TitleRef']/@uriId" />
      <xsl:element name="name">
        <xsl:for-each select="//*[local-name()='CastTitle'][@uriId = $CasturiId]/*[local-name()='PersonNameDetails']">
      ...
      <xsl:call-template name="add_cast_image_thumb"/>
    </xsl:element>  
  </xsl:template>

  <xsl:template name="add_cast_image_thumb">
    <xsl:variable name="UriIdVar" select="*[local-name()='PosterRef']/@uriId"/>
    <xsl:if test="$UriIdVar != ''">
      <xsl:variable name="posterVar" select="..//*[local-name()='Poster'][@uriId = $UriIdVar]/content:SourceUrl"/>
      <xsl:if test="$posterVar != ''">
        <xsl:element name="thumb">
          <xsl:attribute name="url">
            <xsl:value-of select="concat($ImagePathConstVar,$posterVar)" />
          </xsl:attribute>
        </xsl:element>
      </xsl:if>
    </xsl:if>
  ...
  <xsl:template match="//*[local-name()='Title'][@type = 'SeriesTitle']">
    <xsl:element name="media">
      <xsl:attribute name="is_active">true</xsl:attribute>
      <xsl:attribute name="co_guid"><xsl:value-of select="//*[local-name()='Title'][@type = 'ShowTitle']/@uriId"/></xsl:attribute>
      <xsl:attribute name="action">insert</xsl:attribute>
      <xsl:call-template name="build_series_basic_data"/>
      <xsl:call-template name="build_series_structure_data"/>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template name="build_series_tags_data">
    <xsl:element name="metas">
      ...
      <xsl:call-template name="build_ClassificationKey_tag"/>
    </xsl:element>
  </xsl:template>

  <xsl:template name="build_ClassificationKey_tag">
    <xsl:element name="meta">
      <xsl:attribute name="name">ClassificationKey</xsl:attribute>
      <xsl:attribute name="ml_handling">unique</xsl:attribute>
      <xsl:call-template name="set_ClassificationKey"/>
    </xsl:element>
  </xsl:template>

  <xsl:template name="set_ClassificationKey">
    <xsl:for-each select="//*[local-name()='Category']">
      <xsl:element name="container">
        <xsl:element name="value">
          <xsl:attribute name="lang">heb</xsl:attribute>
          <xsl:value-of select="substring-after(@uriId,'yes.co.il/IBMS/category/')"/>
        </xsl:element>
      </xsl:element>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="build_offer_id_tag">
    <xsl:element name="meta">
      <xsl:attribute name="name">OfferID</xsl:attribute>
      <xsl:attribute name="ml_handling">unique</xsl:attribute>
  ...
  <xsl:template name="set_offer_id">
    <xsl:element name="container">
      <xsl:variable name="offerID" select="//*[local-name()='Offer']/@uriId"/>
  ...
  <xsl:template name="build_series_basic_data">
    <xsl:element name="basic">
      <xsl:element name="name">
        <xsl:apply-templates select="//*[local-name()='Title'][@type = 'ShowTitle']/*[local-name()='LocalizableTitle']/title:TitleLong" />
      </xsl:element>
      <xsl:element name="description">
  ...
  <xsl:template name="build_strings_data">
    ...
    <xsl:element name="meta">
      <xsl:attribute name="name">IBMSTitleID</xsl:attribute>
      <xsl:attribute name="ml_handling">unique</xsl:attribute>
      <xsl:element name="value">
        <xsl:attribute name="lang">heb</xsl:attribute>
  ...
  <xsl:template name="build_doubles_data">
    ...
    <xsl:call-template name="build_offer_no" />
    <xsl:call-template name="build_IBMSseriesID" />
    <xsl:call-template name="build_available_seasons" />
  </xsl:template>

  <xsl:template name="build_IBMSseriesID">
    <xsl:element name="meta">
      <xsl:attribute name="name">IBMSseriesID</xsl:attribute>
      <xsl:value-of select="//*[local-name()='Title'][@type = 'SeriesTitle']/@uriId" />
    </xsl:element>
  </xsl:template>

  <xsl:template name="build_available_seasons">
    <xsl:element name="meta">
      <xsl:attribute name="name">Available seasons</xsl:attribute>
      <xsl:text>1</xsl:text>
    </xsl:element>
  </xsl:template>
  ...
  <xsl:template name="add_image_thumb">
    <xsl:variable name="UriIdVar" select="@uriId"/>
    <xsl:variable name="posterRefVar" select="..//*[local-name()='ContentGroup'][offer:TitleRef/@uriId = $UriIdVar]/offer:PosterRef/@uriId"/>
    <xsl:if test="$posterRefVar != ''">
      <xsl:variable name="posterVar" select="..//*[local-name()='Poster'][@uriId = $posterRefVar]/content:SourceUrl"/>
      <xsl:if test="$posterVar != ''">
        <xsl:element name="thumb">
        <xsl:attribute name="url"><xsl:value-of select="concat($ImagePathConstVar,$posterVar)" /></xsl:attribute>
        </xsl:element>
      </xsl:if>
    </xsl:if>
  </xsl:template>
