...
namespace EventBus.RabbitMQ
{
	...
	{
		private const uint MESSAGE_LOG_TTL = 864000; // 10 Days in seconds
		...
		private async Task ConsumerOnReceived(object sender, BasicDeliverEventArgs eventArgs)
		{
			...
			{
				LogEventWithMessage(eventName, message, e);
				consumer.Model.BasicNack(eventArgs.DeliveryTag, multiple: false, requeue: false);
			}
		}
		...
		{
			LogEventWithMessage(eventName, message);
		...
		private static void LogEventWithMessage(string eventName, string message, Exception e = null)
		{
			// default message if less than 1500, otherwise trim and upload full msessage to couchbase
			var msg = $"EventBus.ProcessEvent > eventName:[{eventName}] with message:[{message}]";
			if (message.Length > 1500)
			{
				var key = $"{eventName}_incoming_{KLogger.GetRequestId()}";
				var bucket = CouchbaseManager.eCouchbaseBucket.STATISTICS;
				var cb = new CouchbaseManager.CouchbaseManager(bucket);
				cb.Set(key, message, MESSAGE_LOG_TTL);

				msg = $"ProcessEvent > eventName:[{eventName}] message to large. Sotred in bucket:[{bucket}], with key:[{key}]. First 1500 charecters:[{message.Substring(1500)}]";
			}

			if (e == null) { _Logger.Debug(msg); }
			else { _Logger.Error(msg, e); }
		}
