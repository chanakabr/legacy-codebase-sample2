...
RUN Expand-Archive Core.zip -DestinationPath tmp; mv tmp/$((Get-ChildItem tmp | Select-Object -First 1).Name) Core; rm tmp
...
ADD mv KalturaClient.xml tvpapi_rest\WebAPI\clientlibs\
