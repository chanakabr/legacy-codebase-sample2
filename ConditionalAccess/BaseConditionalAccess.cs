            ...
            string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME, string sClientIP)
        {
            ...
            try
            {
                ...
                if (CachingManager.CachingManager.Exist(sCacheKey))
                    oModules = TVinciShared.ObjectCopier.Clone<TvinciPricing.MediaFilePPVContainer[]>((TvinciPricing.MediaFilePPVContainer[])(CachingManager.CachingManager.GetCachedData(sCacheKey)));
                else
                {
                    ...
                    TvinciPricing.MediaFilePPVContainer[] oModulesCopy = TVinciShared.ObjectCopier.Clone<TvinciPricing.MediaFilePPVContainer[]>(oModules);
       ...
       * The method shall perform a call to the client specific billing gateway to perform a cancellation action on the external billing gateway*/
            ...
            string sRefferer, string sCountryCode, string sLanguageCode, string sDeviceName, string sCouponCode)
        {
            ...
            try
            {
                ...
                {
                    if (!isDevicePlayValid(sSiteGuid, sDeviceName, ref userDomain))
                    {
                        res.altUrl = GetErrorLicensedLink(sBasicLink);
                        res.mainUrl = GetErrorLicensedLink(sBasicLink);
                        res.status = eLicensedLinkStatus.InvalidDevice.ToString();

                        Logger.Logger.Log("GetLicensedLinks", string.Format("Invalid Device, user:{0}, MFID:{1}, device:{2}", 
                            sSiteGuid, nMediaFileID, sDeviceName), GetLogFilename());
                            ...
                            if (IsFreeItem(prices[0]) || IsItemPurchased(prices[0]))
                            {
                                if (sBasicLink.ToLower().EndsWith(fileMainUrl.ToLower())) //(Utils.ValidateBaseLink(m_nGroupID, nMediaFileID, sBasicLink))
                                {
                                    ...
                                    else
                                    {
                                        res.altUrl = GetErrorLicensedLink(sBasicLink);
                                        res.mainUrl = GetErrorLicensedLink(sBasicLink);
                                        res.status = eLicensedLinkStatus.MediaConcurrency.ToString();
