...
namespace ConditionalAccess
{
    class TikleConditionalAccess : TvinciConditionalAccess 
    {
        ...
        protected override void HandleSubscriptionUsesNotification(Int32 nMediaFileID, string sSubCode, string sSiteGUID)
        {
            ...
            Logger.Logger.Log("SubscriptionUsesNotification", sLogMessage, "SubscriptionUsesNotification");
        }

        protected override void HandleCouponUses(TvinciPricing.Subscription relevantSub, string sPPVModuleCode,
            string sSiteGUID, double dPrice, string sCurrency,
            Int32 nMediaFileID, string sCouponCode, string sUserIP,
            string sCountryCd, string sLANGUAGE_CODE, string sDEVICE_NAME, bool bFromPurchase, int nPrePaidCode)
        {
            if (!string.IsNullOrEmpty(sCouponCode))
            {
                ...
                if (sWSURL != "")
                    m.Url = sWSURL;
                m.SetCouponUsed(sWSUserName, sWSPass, sCouponCode, sSiteGUID);
                
                double dPercent = Utils.GetCouponDiscountPercent(m_nGroupID, sCouponCode);
                Logger.Logger.Log("TikleCouponUses", "Set Coupon Used called : " + sCouponCode + ", Coupon Discount Percent : " + dPercent.ToString(), "TikleCouponUses");
                if (dPercent == 100)
                {
                    Logger.Logger.Log("TikleCouponUses", "Coupon Code : " + sCouponCode + ", Coupon Discount Percent : " + dPercent.ToString(), "TikleCouponUses");

                    Int32[] nMediaFiles = { nMediaFileID };
                    string sMediaFileForCache = Utils.ConvertArrayIntToStr(nMediaFiles);
                    TvinciPricing.PPVModule oPPVModule = null;
                    ...
                    if (CachingManager.CachingManager.Exist("GetPPVModuleData" + sMediaFileForCache + "_" + m_nGroupID.ToString() + sLocaleForCache) == true)
                        oPPVModule = (TvinciPricing.PPVModule)(CachingManager.CachingManager.GetCachedData("GetPPVModuleData" + sMediaFileForCache + "_" + m_nGroupID.ToString() + sLocaleForCache));
                    else
                    {
                        oPPVModule = m.GetPPVModuleData(sWSUserName, sWSPass, sPPVModuleCode, sCountryCd, sLANGUAGE_CODE, sDEVICE_NAME);
                    ...
                    //Tickle WS
                    tikle.Service s = new ConditionalAccess.tikle.Service();
                    string sTikleWSURL = Utils.GetWSURL("tikle_ws");
                    s.Url = sTikleWSURL;

                    string sMD5Hash = Utils.GetHash("0" + sTransactionData, "WS_SECRET");
                    try
                    {
                        s.Purchase("0", sTransactionData, sMD5Hash);
                        Logger.Logger.Log("TikleCouponUsesPurchase", sTransactionData, "TikleCouponUses");
                    }
                    catch (Exception ex)
                    {
                        Logger.Logger.Log("TikleCouponUsesException", ex.Message, "TikleCouponUses");
                    }
        ...
        protected override string GetCustomData(TvinciPricing.Subscription relevantSub, TvinciPricing.PPVModule thePPVModule, TvinciPricing.Campaign campaign,
            string sSiteGUID, double dPrice, string sCurrency,
        ...
        protected override string GetCustomDataForSubscription(TvinciPricing.Subscription theSub, TvinciPricing.Campaign campaign, string sSubscriptionCode, string sCampaignCode,
