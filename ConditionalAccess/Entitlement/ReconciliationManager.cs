    ...
    public class ReconciliationManager
    {
        ...
        public static ApiObjects.Response.Status ReconcileEntitlements(BaseConditionalAccess cas, int groupId, string userId)
        {
            ...
            using (API wsApi = new API())
            {
                ...
                Utils.GetWSCredentials(groupId, eWSModules.API, ref wsApiUsername, ref wsApiPass);

                if (string.IsNullOrEmpty(wsApiUsername) || string.IsNullOrEmpty(wsApiPass))
                {
                    log.ErrorFormat("ReconcileEntitlements: failed to get WS API credentials. groupId = {0}, userId = {1}", groupId, userId);
                    return response;
                }
        ...
        private static Dictionary<long, PPVModule> GetPpvModulesDataForExternalEntitlements(int groupId, string userId, ref List<ExternalEntitlement> ppvEntitlementsToInsert)
        {
            ...
            if (ppvEntitlementsToInsert.Count > 0)
            {
                using (mdoule wsPricing = new mdoule())
                {
                    ...
                    Utils.GetWSCredentials(groupId, eWSModules.PRICING, ref wsPricingUsername, ref wsPricingPass);

                    if (string.IsNullOrEmpty(wsPricingUsername) || string.IsNullOrEmpty(wsPricingPass))
                    {
                        log.ErrorFormat("ReconcilePPVs: failed to get WS Pricing credentials or URL. groupId = {0}, userId = {1}", groupId, userId);
                    }
        ...
        private static Dictionary<long, Subscription> GetSubscriptionsDataForExternalEntitlements(int groupId, string userId, ref List<ExternalEntitlement> subscriptionEntitlementsToInsert)
        {
            ...
            if (subscriptionEntitlementsToInsert.Count > 0)
            {
                using (mdoule wsPricing = new mdoule())
                {
                    ...
                    if (string.IsNullOrEmpty(wsPricingUsername) || string.IsNullOrEmpty(wsPricingPass))
                    {
                        log.ErrorFormat("ReconcileSubscriptions: failed to get WS Pricing credentials or URL. groupId = {0}, userId = {1}", groupId, userId);
                    }
