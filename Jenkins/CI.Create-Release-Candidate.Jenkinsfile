...
node('Linux'){
    ...
    stage('Check Success Artifacts'){
        ...
        def result = sh (script: "sudo ./RC-GetFailedBuilds.sh ${BRANCH_NAME} build", returnStatus: true)
        if (result == 1){
            def report = sh (script: "sudo ./ReportJobStatus.sh ${BRANCH_NAME} rc ${env.BUILD_NUMBER} ${env.JOB_NAME} rc FAILURE ", returnStdout: true)
            echo "${report}"    
        }
    }
        // for (job in status){
