...
namespace ElasticSearch.Common
{
    public class ElasticSearchApi
    {
        ...
        private static readonly string ES_LOG_FILENAME = "ElasticSearch";
        ...
        public bool InsertMapping(string sIndex, string sMapName, string sMappingObject)
        {
            ...
            {
                string sUrl = string.Format("{0}/{1}/{2}/_mapping", ES_URL, sIndex, sMapName);
        ...
        public void DeleteIndices(List<string> lIndices)
        {
            if (lIndices != null)
            {
                ...
                foreach (string sIndex in lIndices)
                {
                    ...
                    if (nStatus != 200)
                        Logger.Logger.Log("Error", string.Format("Unable to delete index. index={0}", sIndex), ES_LOG_FILENAME);
                }
        ...
        public bool InsertRecord(string sIndex, string sType, string sID, string sDoc)
        {
            ...
            bRes = (nStatus == 200) ? true : false;
            if (!bRes)
                Logger.Logger.Log("Error", string.Format("Unable to insert record into elasticsearch. url={0};document={1}", sUrl, sDoc), ES_LOG_FILENAME);
        ...
        public string Search(string sIndex, string sType, ref string sSearchQuery, List<string> routing = null)
        {
            ...
            if (routing != null && routing.Count > 0)
            {
                ...
                sUrl = string.Format("{0}/{1}/{2}/_search?routing={3}", ES_URL, sIndex, sType,sRouting);
            }
            else
            {
                sUrl = string.Format("{0}/{1}/{2}/_search", ES_URL, sIndex, sType);
            }
            int nStatus = 0;
            ...
            if (nStatus != 200)
            {
                Logger.Logger.Log("Error", string.Format("Search query failed. url={0};query={1}", sUrl, sSearchQuery), ES_LOG_FILENAME);
                sRes = string.Empty;
            }
        ...
        public string MultiSearch(string sIndex, string sType, List<string> lSearchQueries)
        {
            ...
            if (nStatus != 200)
            {
                Logger.Logger.Log("Error", string.Format("Search query failed. url={0};query={1}", sUrl, sb.ToString()), ES_LOG_FILENAME);
                sRes = string.Empty;
            }
        ...
        public List<string> SearchPercolator(string sIndex, string sType, ref string sDoc)
        {
            ...
            if (nStatus != 200)
            {
                Logger.Logger.Log("Error", string.Format("Search Percolator query failed. url={0};doc={1}", sUrl, sDoc), ES_LOG_FILENAME);
            }
            else
            {
                try
        ...
        public bool AddQueryToPercolator(string sIndex, string sQueryName, ref string sQuery)
        {
            ...
            {
                Logger.Logger.Log("Error", string.Format("Adding Query to Percolator failed. url={0};query={1}", sUrl, sQuery), ES_LOG_FILENAME);
            }
            ...
            if (!string.IsNullOrEmpty(sIndex) && !string.IsNullOrEmpty(sType) && oIDsList != null && oIDsList.Count > 0)
            {
                ...
                if (sQuery.Length > 0)
                {
                    ...
                    if (nHttpStatusCode != 200)
                    {
                        res = string.Empty;
                        #region Logging
                        Logger.Logger.Log("MultiGetIDs", string.Format("Http status code: {0} , Index: {1} , Type: {2} , typeof(T) : {3} Query sent: {4}", nHttpStatusCode, sIndex, sType, oIDsList[0].GetType().Name, sQuery), ES_LOG_FILENAME);
                        #endregion
                    }
                }
            }
