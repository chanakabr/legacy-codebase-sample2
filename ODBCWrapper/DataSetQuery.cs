    ...
    public class DataSetQuery : Query
    {
        ...
        protected virtual System.Data.DataTable ExecuteQuery(string oraStr, string sVirtualTableName, bool bForceQuery)
        {
            ...
            if (dCached == null)
            {
                ...
                adapter = null;
                if (this.GetType() == System.Type.GetType("TVPApi.ODBCWrapper.DataSetInsertQuery"))
                {
                    m_myDataSet.Tables.Add(sVirtualTableName);
                }
                if (m_nCachedSec > 0)
                    SelectCacher.SetCachedDataTable(sCachStr, m_myDataSet.Tables[sVirtualTableName]);
            }
            else
            {
