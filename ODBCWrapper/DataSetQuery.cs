        ...
        protected virtual System.Data.DataTable ExecuteQuery(string oraStr, string sVirtualTableName, bool bForceQuery)
        {
            ...
            if (dCached == null)
            {
                ...
                using (SqlConnection con = new SqlConnection(sConn))
                {
                    ...
                    try
                    {
                        using (KMonitor km = new KMonitor(KLogMonitor.Events.eEvent.EVENT_DATABASE, null, null, null, null) { Database = command.Connection.Database, QueryType = Events.eDBQueryType.UPDATE })
                        {
