            ...
            if (numOfRequests == 1 && !m_oEpgSearchObj.m_bSearchOnlyDatesAndChannels)
            {
                ...
                foreach (SearchValue kvp in m_oEpgSearchObj.m_lSearchOr)
                {
                    if (!m_oEpgSearchObj.m_bExact)
                    {
                        ...
                        {
                            Key = ElasticSearch.Common.Utils.ReplaceQueryReservedCharacters(kvp.m_sKey),
                            Value = string.Format("*{0}*", ElasticSearch.Common.Utils.ReplaceQueryReservedCharacters(kvp.m_sValue))
                        };
                    }
                    else
                    {
