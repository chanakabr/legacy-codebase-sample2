    ...
    public class VodafoneConditionalAccess : TvinciConditionalAccess
    {
        ...
        private List<int> GetFileList(List<int> fileIds, ref bool priceValidationPassed)
        {
            try
            {
                ...
                oModules = Pricing.Module.GetPPVModuleListForMediaFilesWithExpiry(m_nGroupID, fileIds.ToArray(), string.Empty, string.Empty, string.Empty);
        ...
        private string GetEpgProgramCoGuid(string assetID, ref string epgChannelID, ref DateTime startDate)
        {
            ...
            EpgProgramDetailsRequest epdr = new EpgProgramDetailsRequest();
            epdr.m_nGroupID = m_nGroupID;
            epdr.m_oFilter = new Filter();
            epdr.m_lProgramsIds = new List<int> { progID };
            Utils.FillCatalogSignature(epdr);
            EpgProgramResponse resp = epdr.GetProgramsByIDs(epdr) as EpgProgramResponse;
            if (resp != null && resp.m_lObj != null && resp.m_lObj.Count > 0 && resp.m_lObj[0] != null)
            {
                ProgramObj prog = resp.m_lObj[0] as ProgramObj;
