    ...
    public class EpgRequest : BaseRequest, IRequestImp, IEpgSearchable
    {
        ...
        public override BaseResponse GetResponse(BaseRequest oBaseRequest)
        {
            ...
            try
            {
                ...
                sro = CatalogLogic.GetProgramIdsFromSearcher(BuildEPGSearchObject());

                if (sro != null && sro.m_resultIDs != null && sro.m_resultIDs.Count > 0)
                {
                    response = CatalogLogic.GetEPGProgramsFromCB(sro.m_resultIDs.Select(item => item.assetID).ToList<int>(), m_nGroupID, m_eSearchType == EpgSearchType.Current, m_nChannelIDs, oBaseRequest.m_oFilter.m_nLanguage);
                }
            }
            catch (Exception ex)
            {
