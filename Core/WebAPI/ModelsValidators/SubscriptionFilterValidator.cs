    ...
    public static class SubscriptionFilterValidator
    {
        public static void Validate(this KalturaSubscriptionFilter model)
        {
            if (string.IsNullOrEmpty(model.Ksql))
            {
                ...
                if (!string.IsNullOrEmpty(model.SubscriptionIdIn) && (model.MediaFileIdEqual.HasValue || !string.IsNullOrEmpty(model.ExternalIdIn) || 
                ...
                if (!string.IsNullOrEmpty(model.ExternalIdIn) && (model.MediaFileIdEqual.HasValue || !string.IsNullOrEmpty(model.SubscriptionIdIn) || model.AlsoInactive.HasValue ||
                                                                 model.PreviewModuleIdEqual.HasValue || model.PricePlanIdEqual.HasValue || model.ChannelIdEqual.HasValue))
