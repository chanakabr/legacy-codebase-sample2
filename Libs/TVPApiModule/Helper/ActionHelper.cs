...
namespace TVPApi
{
    public class ActionHelper
    {
        ...
        {
            return new TVPPro.SiteManager.CatalogLoaders.MediaMarkLoader(groupID, SiteHelper.GetClientIP(), initObj.SiteGuid, initObj.UDID, (int)fileParams.mediaID, (int)fileParams.fileID, npvrID, fileParams.avg_bit_rate_num, fileParams.current_bit_rate_num, iLocation, fileParams.total_bit_rate_num, Action.ToString(), fileParams.duration, string.Empty, string.Empty, string.Empty)
            {
                Platform = platform.ToString()
            }.Execute() as string;
        ...
        public static string MediaHit(InitializationObject initObj, int groupID, PlatformType platform, int mediaType, long iMediaID, long iFileID, int iLocation, string npvrID)
        {
            return new TVPPro.SiteManager.CatalogLoaders.MediaHitLoader(groupID, SiteHelper.GetClientIP(), initObj.SiteGuid, initObj.UDID, (int)iMediaID, (int)iFileID, npvrID, 0, 0, iLocation, 0, string.Empty, string.Empty)
            {
                Platform = platform.ToString()
            }.Execute() as string;
        }
        ...
        public static void MediaError(InitializationObject initObj, int groupID, PlatformType platform, int mediaType, long iMediaID, long iFileID, int iLocation, string sErrorCode, string sErrorMessage, string npvrID)
        {
            new TVPPro.SiteManager.CatalogLoaders.MediaMarkLoader(groupID, SiteHelper.GetClientIP(), initObj.SiteGuid, initObj.UDID, (int)iMediaID, (int)iFileID, npvrID, 0, 0, iLocation, 0, string.Empty, string.Empty, sErrorCode, sErrorMessage, string.Empty)
            {
                Platform = platform.ToString()
            }.Execute();
        }

        public static void MediaError(InitializationObject initObj, int groupID, PlatformType platform, FileHolder fileParams, int iLocation, string sErrorCode, string sErrorMessage, string npvrID)
        {
