...
namespace Tvinci.Core.DAL
{
    public class CatalogDAL : BaseDal
    {
        ...
        public static List<UserMediaMark> Get_PersonalLastDevice(List<int> nMediaIDs, string userId)
        {
            ...
            lRes = mediaMarkLogList.OrderByDescending(x => x.LastMark.CreatedAt).Select(x => x.LastMark).ToList();
        ...
        public static List<UserMediaMark> GetMediaMarksLastDateByUsers(List<int> usersList)
        {
            ...
            foreach (var row in res)
            {
                ...
                {
                    string userIdString = row.Key.ToString();

                    if (userIdString.Contains("u"))
                    {
                        userIdString = userIdString.Replace("u", "");
                    }

                    int.TryParse(userIdString, out nUserID);

                    List<AssetAndLocation> userMediaMarks = row.Value.mediaMarks;
        ...
        public static List<UserMediaMark> GetMediaMarksLastDateByMedias(List<int> mediasList)
        {
            ...
            if (res != null)
            {
                ...
                {
                    mediasMarksList = GetMediaMarksLastDateByUsers(userList);
                }
            }
            return mediasMarksList;
        ...
        public static int GetMediaConcurrencyRuleIdPlayCycleKey(string sSiteGuid, int nMediaID, int nMediaFileID, string sUDID, int nPlatform)
        {
            StoredProcedure sp = new StoredProcedure("GetRuleIDPlayCycleKey");
            sp.SetConnectionKey("MAIN_CONNECTION_STRING");
        ...
        public static void UpdateOrInsertUsersMediaMark(UserMediaMark userMediaMark, bool isFirstPlay, int finishedPercentThreshold, bool isLinearChannel)
        {
            ...
            if (isFirstPlay || shouldUpdateLocation)
            {
                ...
                while (limitRetries >= 0 && !success)
                {
                    UpdateOrInsertUsersMediaMarkOrHit(mediaMarksManager, ref limitRetries, r, mmKey, ref success, userMediaMark, isFirstPlay, 0);
                }
            }
        }
        ...
        private static bool UpdateOrInsertUsersMediaHit(ref bool success, UserMediaMark userMediaMark, int finishedPercentThreshold)
        {
            ...
            var mediaHitData = couchbaseManager.Get<string>(mmKey);
            ...
            bool result = couchbaseManager.Set(mmKey, JsonConvert.SerializeObject(umm, Formatting.None));
        ...
        public static void UpdateOrInsertUsersNpvrMark(UserMediaMark userNpvrMark, bool isFirstPlay)
        {
            ...
            {
                shouldUpdateLocation = UpdateOrInsertUsersMediaHit(ref hitSuccess, userNpvrMark, 95);
            }
            ...
            if (isFirstPlay || shouldUpdateLocation)
            {
                ...
                {
                    UpdateOrInsertUsersMediaMarkOrHit(mediaMarkManager, ref limitRetries, r, mmKey, ref markSuccess, userNpvrMark, isFirstPlay, 0);
                }
            }
        }
        ...
        public static void UpdateOrInsertDevicePlayData(DevicePlayData devicePlayData, bool isReportingMode, eExpirationTTL ttl)
        {
            if (!isReportingMode)
            {
                ...
                {
                    if (!SaveDevicePlayData(devicePlayData, ttl))
                    {
                        Thread.Sleep(r.Next(50));
                        limitRetries--;
                    }
        ...
        private static bool UpdateOrInsertUsersMediaMarkOrHit(CouchbaseManager.CouchbaseManager couchbaseManager, ref int limitRetries, Random r,
                                                              string mmKey, ref bool success, UserMediaMark userMediaMark, bool isFirstPlay, int finishedPercent = 95)
        {
            bool userMediaMarksSuccess = true;
            bool locationStatusChanged = false;
            int previousLocation = 0;
            uint expiration = (uint)ApplicationConfiguration.MediaMarksTTL.IntValue * 60 * 60 * 24;

            string userAllAssetMarksDocumentKey = UtilsDal.GetUserAllAssetMarksDocKey(userMediaMark.UserID.ToString());

            List<string> keysToGet = new List<string>() { mmKey };

            if (isFirstPlay)
            {
                keysToGet.Add(userAllAssetMarksDocumentKey);
            }

            IDictionary<string, string> dictionaryValues = couchbaseManager.GetValues<string>(keysToGet, true);

            UserMediaMarks mediaMarks  = null;
            string mediaHitData = string.Empty;

            #region User Media Marks

            if (isFirstPlay)
            {
                if (dictionaryValues.ContainsKey(userAllAssetMarksDocumentKey))
                {
                    try
                    {
                        mediaMarks = JsonConvert.DeserializeObject<UserMediaMarks>(dictionaryValues[userAllAssetMarksDocumentKey]);
                    }
                    catch (Exception ex)
                    {
                        log.ErrorFormat("Error parsing user media marks data for key {0} error = {1}", userAllAssetMarksDocumentKey, ex);
                    }
                }
                ...
                userMediaMarksSuccess = false;
                userMediaMarksSuccess = couchbaseManager.Set(userAllAssetMarksDocumentKey, mediaMarks, expiration);
            ...
            #region Media Mark Log

            if (dictionaryValues.ContainsKey(mmKey))
            {
                mediaHitData = dictionaryValues[mmKey];
            }
        ...
        public static DomainMediaMark GetAssetLastPosition(string assetID, eAssetTypes assetType, List<int> users)
        {
            ...
            switch (assetType)
            {
                case eAssetTypes.EPG:
                    long epgId;
                    if (long.TryParse(assetID, out epgId))
                    {
                        foreach (int userID in users)
                        {
                            userDocKey = UtilsDal.GetUserEpgMarkDocKey(userID, epgId);
                ...
                case eAssetTypes.MEDIA:
                    ...
                    if (int.TryParse(assetID, out mediaID))
                    {
                        foreach (int userID in users)
                        {
                            userDocKey = UtilsDal.GetUserMediaMarkDocKey(userID.ToString(), mediaID);
                            userKeys.Add(userDocKey);
                        }
                    }
                    break;
        ...
        public static void GetGroupDefaultParameters(int groupId, out bool isRegionalizationEnabled, out int defaultRegion, out int defaultRecommendationEngine,
                                                     ...
                                                     out bool isGeoAvailabilityEnabled, out bool isAssetUserRuleEnabled)
        {
            isRegionalizationEnabled = false;
            defaultRegion = 0;
            ...
            if (groupDataSet != null && groupDataSet.Tables != null && groupDataSet.Tables.Count == 1)
            {
                ...
                if (groupTable != null && groupTable.Rows != null && groupTable.Rows.Count > 0 && groupTable.Rows[0] != null)
                {
                    ...
                    isAssetUserRuleEnabled = ODBCWrapper.Utils.ExtractBoolean(groupRow, "IS_ASSET_USER_RULE_ENABLED");
                }
            }
        }
        ...
        public  static KSQLChannel CreateKSQLChannelByDataRow(DataTable assetTypesTable, DataRow rowData, Dictionary<string, string> metas = null, bool doesGroupUsesTemplates = false)
        {
            ...
            if ((status == 1) && (channelType == 4))
            {
                ...
                if (orderBy == 100 && doesGroupUsesTemplates)
                {
                    channel.Order.m_sOrderValue = orderByValue;
                    channel.Order.m_eOrderBy = ApiObjects.SearchObjects.OrderBy.META;
                }
                // all META_STR/META_DOUBLE values
                else if (orderBy >= 1 && orderBy <= 30)
                {
        ...
        public static string GetOrInsertPlayCycleKey(string siteGuid, long mediaID, long mediaFileID, string udid, long platform,
        ...
        /// <param name="mcRuleID"></param>
        /// <param name="groupID"></param>
        /// <param name="playCycleKey"></param>
        public static void InsertPlayCycleKey(string siteGuid, long mediaID, long mediaFileID, string udid, long platform, long countryID,
                                              int mcRuleID, int groupID, string playCycleKey)
        {
        ...
        private static List<ApiObjects.Meta> CreateTopicInterestList(DataTable result)
        {
            ...
            {
                meta = CreateTopicInterestMeta(result.Rows[rowIndex]);
        ...
        private static ApiObjects.Meta CreateTopicInterestMeta(DataRow dr)
        {
            int assetType = 0;
        ...
        public static DataTable GetImagesByIds(int groupId, List<long> imageIds, bool? isDefault = null)
        {
            ...
            bool? isActive, DateTime? catalogEndDate)
        {
            ...
            sp.AddParameter("@updaterId", userId);
            sp.AddParameter("@assetId", assetId);
            sp.AddParameter("@type", type.Value);
            sp.AddParameter("@url", url);
            sp.AddParameter("@streamingSuplierId", streamingSuplierId ?? 0);
            sp.AddParameter("@altStreamingCode", altStreamingCode);
            sp.AddParameter("@altStreamingSuplierId", altStreamingSuplierId ?? 0);
            sp.AddParameter("@additionalData", additionalData);
            sp.AddParameter("@billingType", billingType);
            sp.AddParameter("@orderNum", orderNum ?? 0);
            sp.AddParameter("@duration", duration ?? 0);
        ...
        public static DataSet UpdateTopicAssets(int groupId, List<long> tagTopicIds, bool shouldDeleteTag, bool shouldDeleteAssets, List<long> MetaTopicIds, long assetStructId, long userId)
        {
            ...
            sp.SetConnectionKey("MAIN_CONNECTION_STRING");
            sp.AddParameter("@GroupId", groupId);
            sp.AddParameter("@MetaTopicExist", MetaTopicIds != null && MetaTopicIds.Count > 0 ? 1 : 0);
            sp.AddIDListParameter<long>("@MetaTopicIds", MetaTopicIds, "Id");
        ...
        public static DataSet GetMediaFilesByExternalIdAndAltExternalId(int groupId, string externalId, string altExternalId)
        {
            ...
            sp.AddParameter("@altExternalIdExists", string.IsNullOrEmpty(altExternalId) ? 0 : 1);

            return sp.ExecuteDataSet();
        }

        public static DataSet GetGroupChannels(int groupId)
        {
            ODBCWrapper.StoredProcedure sp = new ODBCWrapper.StoredProcedure("GetGroupChannels");
            sp.SetConnectionKey("MAIN_CONNECTION_STRING");
            sp.AddParameter("GroupId", groupId);
            DataSet ds = sp.ExecuteDataSet();
