...
namespace Playground
{
    class Program
    {
        ...
        {
            var workingDir = @"c:\src\ott-backend-sonar\";

            var projects = GetProjects(workingDir).ToList();
            GenerateCSV(projects, workingDir);

            var exclude = new[] { "ApiLogicNetCore", "ImageManagerCore" };
            var dotnetCoreProjects = projects.Where(_ => _.NetCore && !exclude.Any(e => _.fileName.Contains(e)) );
            var netCoreSln = GenerateSln("ott-be-net-core", Path.Combine("sonarqube", "netcore"), workingDir, dotnetCoreProjects);
            Validate(netCoreSln, workingDir);

            var exclude = new [] { "ApiLogicNetCore", "ImageManagerCore", "Tester", Path.Combine("tvpapi","QueueWrapper"), "Tvinci.Web.Controls", "Tvinci.Web.TVS.Controls",
            ...
            var dotnetFrameworkProjects = projects.Where(_ => _.NetFramework && !exclude.Any(e => _.fileName.Contains(e, StringComparison.OrdinalIgnoreCase)));
            var netFrameworkSln = GenerateSln("ott-be-net-framework", Path.Combine("sonarqube", "netframework"), workingDir, dotnetFrameworkProjects);
