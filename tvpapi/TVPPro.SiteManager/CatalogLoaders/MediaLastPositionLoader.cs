    ...
    public class MediaLastPositionLoader : CatalogRequestManager
    {
        ...
        private MediaMarkObject ResponseToMediaMarkObject()
        {
            ...
            if (m_oResponse != null)
            {
                ...
                {
                    eStatus = response.m_sStatus != "BAD_REQUEST" || response.m_sStatus != "INVALID_PARAMS" ? 
                    MediaMarkObject.MediaMarkObjectStatus.OK :
                    MediaMarkObject.MediaMarkObjectStatus.FAILED,
                    nGroupID = GroupID,
