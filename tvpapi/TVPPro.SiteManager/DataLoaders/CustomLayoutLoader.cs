...
namespace TVPPro.SiteManager.DataLoaders
{
    ...
    public class CustomLayoutLoader : CustomAdapter<TVPPro.SiteManager.DataLoaders.CustomLayoutLoader.CustomLayout>
    {
        ...
        {
            DataTable dt = new DatabaseDirectAdapter(delegate(TVPApi.ODBCWrapper.DataSetSelectQuery query)
            {
                ...
                query += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("CustomLayoutItemTypeID", "=", (int)ItemType);
                query += "and";
                query += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("LanguageID", "=", TextLocalization.Instance.UserContext.ValueInDB);

                if (ItemID > 0)
                {
                    query += "and";
                    query += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("ItemID", "=", ItemID);
                }
