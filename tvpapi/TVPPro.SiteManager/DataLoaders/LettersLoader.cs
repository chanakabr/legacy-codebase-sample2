...
namespace TVPPro.SiteManager.DataLoaders
{
	...
	public class LetterLoader : CustomAdapter<DataTable>
	{
		...
		{
			DataTable table = new DatabaseDirectAdapter(delegate(TVPApi.ODBCWrapper.DataSetSelectQuery query)
			{
				query += "select sbl.letter 'Letter' from SearchByLetters sbl where ";
				query += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("sbl.languageid", "=", TextLocalization.Instance.UserContext.ValueInDB);
