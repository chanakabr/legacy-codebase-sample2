...
namespace TVPPro.SiteManager.Services
{
    public class ConditionalAccessService
    {
            ...
            try
            {
                switch (type)
                {
                    case ePaymentType.PPV:
                        billingResponse = Core.ConditionalAccess.Module.PU_GetPPVPopupPaymentMethodURL(groupId, sSiteGUID, dPrice, sCurrencyCode, iMediaFileID, sModuleCode, couponCode, StringEnum.GetStringValue(method), SiteHelper.GetSiteBaseURL(true), sLocaleCountry, sLocaleLanguage, sLocaleDevice);
                        break;
                    case ePaymentType.Package:
                        billingResponse = Core.ConditionalAccess.Module.PU_GetSubscriptionPopupPaymentMethodURL(groupId, sSiteGUID, dPrice, sCurrencyCode, iMediaFileID.ToString(), couponCode, StringEnum.GetStringValue(method), SiteHelper.GetSiteBaseURL(true), sLocaleCountry, sLocaleLanguage, sLocaleDevice);
                        break;
                }
            }
            catch (Exception ex)
        ...
        public PermittedMediaContainer[] GetUserPermittedItems(string sID)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.GetUserPermittedItems(groupId, sID);
        ...
        public PermittedMediaContainer[] GetUserExpiredItems(string sID, int NumOfItems)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.GetUserExpiredItems(groupId, sID, NumOfItems);
        ...
        public PermittedMediaContainer[] GetUserPermittedItems(string sID, string wsUser, string wsPass)
        {
            ...
            try
            {
                int groupID = Core.ConditionalAccess.Utils.GetGroupID(wsUser, wsPass);
        ...
        public bool IsUserPermittedItem(string sID, int mediaID)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.IsPermittedItem(groupId, sID, mediaID);
        ...
        public bool IsUserPermittedSubscription(string sID, int subID, ref string reason)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.IsPermittedSubscription(groupId, sID, subID, ref reason);
        ...
        public UserCAStatus GetUserCAStatus(string siteGuid)
        {
            ...
            int groupID = Utils.GetGroupID(wsUser, wsPass);

            if (!string.IsNullOrEmpty(siteGuid))
            {
                try
                {
                    retVal = Core.ConditionalAccess.Module.GetUserCAStatus(groupID, siteGuid);
        ...
        public UserCAStatus GetUserCAStatus()
        {
            ...
            object oFromCache = CachingManager.CachingManager.GetCachedData(sKey);
                //HttpRuntime.Cache[sKey];
            ...
            if (UsersService.Instance.UserContext.UserResponse != null && UsersService.Instance.UserContext.UserResponse.m_user != null)
            {
                ...
                try
                {
                    ...
                    CachingManager.CachingManager.SetCachedData(sKey, retVal, 30, System.Runtime.Caching.CacheItemPriority.Default, 0, true);
        ...
        public SubscriptionsPricesContainer[] GetSubscriptionPricesWithCoupon(string sSubscriptions, string couponCode, bool LowerPrice)
        {
            ...
            try
            {
                returnObject = Core.ConditionalAccess.Module.GetSubscriptionsPricesSTWithCoupon(groupId, sSubscriptions, UsersService.Instance.GetUserID(), couponCode, string.Empty, string.Empty, string.Empty);
            }
            catch (Exception ex)
            {
        ...
        public SubscriptionsPricesContainer[] GetSubscriptionsPrices(string guid, string[] sSubscriptions, bool LowerPrice)
        {
            ...
            try
            {
                returnObject = Core.ConditionalAccess.Module.GetSubscriptionsPrices(groupId, sSubscriptions, guid, string.Empty, string.Empty, string.Empty, SiteHelper.GetClientIP());
        ...
        public SubscriptionsPricesContainer[] GetSubscriptionsPrices(string[] sSubscriptions, bool LowerPrice)
        {
            ...
            try
            {
                returnObject = Core.ConditionalAccess.Module.GetSubscriptionsPrices(groupId, sSubscriptions, UsersService.Instance.GetUserID(), string.Empty, string.Empty, string.Empty, SiteHelper.GetClientIP());
            }
            catch (Exception ex)
            {
                logger.ErrorFormat("Error calling webservice protocol : GetSubscriptionsPrices, Error Message: {0}, Parameters : User: {1}", ex.Message, UsersService.Instance.GetUserID());
        ...
        public Dictionary<int, MediaFileItemPricesContainer> GetItemsPrice(int[] MediaFiles, bool LowestPrice)
        {
            ...
            try
            {
                if (SessionHelper.LocaleInfo != null)
                {
                    res = Core.ConditionalAccess.Module.GetItemsPrices(groupId, MediaFiles, UsersService.Instance.GetUserID(), LowestPrice, SessionHelper.LocaleInfo.LocaleCountry.ToString(), SessionHelper.LocaleInfo.LocaleLanguage, SessionHelper.LocaleInfo.LocaleDevice, SiteHelper.GetClientIP());
                }
                else
                {
                    res = Core.ConditionalAccess.Module.GetItemsPrices(groupId, MediaFiles, UsersService.Instance.GetUserID(), LowestPrice, string.Empty, string.Empty, string.Empty, SiteHelper.GetClientIP());
                }
        ...
        public Dictionary<int, MediaFileItemPricesContainer> GetItemsPrice(int[] MediaFiles, string wsUser, string wsPass, string userGuid, bool LowestPrice)
        {
            ...
            {
                int groupID = Utils.GetGroupID(wsUser, wsPass);
                res = Core.ConditionalAccess.Module.GetItemsPrices(groupID, MediaFiles, userGuid, LowestPrice, string.Empty, string.Empty, string.Empty, SiteHelper.GetClientIP());
        ...
        public Dictionary<int, MediaFileItemPricesContainer> GetItemsPriceWithCoupon(int[] iMediaFiles, string sCouponCode, bool bOnlyLowest)
        {
            ...
            try
            {
                if (SessionHelper.LocaleInfo != null)
                {
                    var res = Core.ConditionalAccess.Module.GetItemsPricesWithCoupons(groupId, iMediaFiles, UsersService.Instance.GetUserID(), sCouponCode, bOnlyLowest, SessionHelper.LocaleInfo.LocaleCountry.ToString(), SessionHelper.LocaleInfo.LocaleLanguage, SessionHelper.LocaleInfo.LocaleDevice, SiteHelper.GetClientIP());
                    if (res != null)
                        result = res.ItemsPrices;
                }
                else
                {
                    var res = Core.ConditionalAccess.Module.GetItemsPricesWithCoupons(groupId, iMediaFiles, UsersService.Instance.GetUserID(), sCouponCode, bOnlyLowest, string.Empty, string.Empty, string.Empty, SiteHelper.GetClientIP());
                    if (res != null)
                        result = res.ItemsPrices;
                }
            }
        ...
        public PermittedSubscriptionContainer[] GetUserPermitedSubscriptions(string guid)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.GetUserPermittedSubscriptions(groupId, guid);
        ...
        public PermittedSubscriptionContainer[] GetUserExpiredSubscriptions(string guid, int numOfItems)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.GetUserExpiredSubscriptions(groupId, guid, numOfItems);
        ...
        public bool CancelSubscription(string sUserGuid, string sSubID, Int32 nSubPurchaseID)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.CancelSubscription(groupId, sUserGuid, sSubID, nSubPurchaseID);
        ...
        public bool RenewCancledSubscription(string sUserGuid, string sSubID, Int32 nSubPurchaseID)
        {
            ...
            try
            {
                res = Core.ConditionalAccess.Module.RenewCancledSubscription(groupId, sUserGuid, sSubID, nSubPurchaseID);
        ...
        public BillingResponse ChargeUser_ForMediaFile(double iPrice, string sCurrency, int iFileID, string sPPVModuleCode, string sUserIP, string sCouponCode, string sCCVariant, ref string sExternalReceiptCode, ref string sReceiptCode)
        {
            ...
            try
            {
                ...
                if (sCCVariant == "M1")
                {
                    response = Core.ConditionalAccess.Module.Cellular_ChargeUserForMediaFile(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, iFileID, sPPVModuleCode, sCouponCode, sUserIP, "", sLocaleCountry, sLocaleLanguage, sLocaleDevice);
                }
                else
                {
                    var res = Core.ConditionalAccess.Module.CC_ChargeUserForMediaFile(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, iFileID, sPPVModuleCode, sCouponCode, sUserIP, "", sLocaleCountry, sLocaleLanguage, sLocaleDevice, string.Empty, string.Empty);
                    if (res != null)
                    {
        ...
        public BillingResponse Dummy_ChargeUserForMediaFile(double iPrice, string sCurrency, int iFileID, string sPPVModuleCode, string sUserIP, string couponCode, string sExtraParameters)
        {
            ...
            try
            {
                ...
                response = Core.ConditionalAccess.Module.CC_DummyChargeUserForMediaFile(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, iFileID, sPPVModuleCode, couponCode, sUserIP, sExtraParameters, sLocaleCountry, sLocaleLanguage, sLocaleDevice);
            }
            catch (Exception ex)
            {
                logger.ErrorFormat("Error calling webservice protocol : ChargeUserForMediaFile, Error Message: {0}, Parameters :  User: {1}", ex.Message, UsersService.Instance.GetUserID());
        ...
        public string DummyChargeUserForPrePaid(double iPrice, string sCurrency, string iFileID, string sPPVModuleCode, string sUserIP, string couponCode, string sExtraParameters)
        {
            ...
            try
            {
                ...
                response = Core.ConditionalAccess.Module.CC_DummyChargeUserForPrePaid(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, iFileID, couponCode, sUserIP, sExtraParameters, sLocaleCountry, sLocaleLanguage, sLocaleDevice);
            }
            catch (Exception ex)
            {
        ...
        public BillingTransactionsResponse GetUserTransactionHistory(int startIndex, int count)
        {
            ...
            try
            {
                BillingTransactions response = Core.ConditionalAccess.Module.GetUserBillingHistory(groupId, UsersService.Instance.GetUserID(), 
                    startIndex, count, TransactionHistoryOrderBy.CreateDateDesc, DateTime.MinValue, DateTime.MaxValue);
                if (response != null)
                {
        ...
        public BillingResponse ChargeUser_ForSubscription(double iPrice, string sCurrency, string sSubscriptionID, string sUserIP, string sCouponCode, string sCCVariant, ref string sExternalCode, ref string sReceiptCode, string sExtraParams)
        {
            ...
            try
            {
                ...
                if (sCCVariant == "M1")
                {
                    response = Core.ConditionalAccess.Module.Cellular_ChargeUserForSubscription(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, sSubscriptionID, sCouponCode, sUserIP, sExtraParams, sLocaleCountry, sLocaleLanguage, sLocaleDevice);
                }
                else
                {
                    var res = Core.ConditionalAccess.Module.CC_ChargeUserForSubscription(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, sSubscriptionID, sCouponCode, sUserIP, sExtraParams, sLocaleCountry, sLocaleLanguage, sLocaleDevice, string.Empty, string.Empty);
                    if (res != null)
                    {
                        response = res.BillingResponse;
                    }
        ...
        public string ChargeUserForSubscription(double iPrice, string sCurrency, string sSubscriptionID, string sUserIP, string sCouponCode)
        {
            ...
            try
            {
                ...
                var res = Core.ConditionalAccess.Module.CC_ChargeUserForSubscription(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, sSubscriptionID, sCouponCode, sUserIP, "", sLocaleCountry, sLocaleLanguage, sLocaleDevice, string.Empty, string.Empty);
                if (res != null)
                {
                    response = res.BillingResponse;
                }
        ...
        public string ChargeUserForPrepaid(double iPrice, string sCurrency, string sPrepaidId, string sUserIP, string sCouponCode)
        {
            ...
            try
            {
                ...
                response = Core.ConditionalAccess.Module.CC_ChargeUserForPrePaid(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, sPrepaidId, sCouponCode, sUserIP, "", sLocaleCountry, sLocaleLanguage, sLocaleDevice);
        ...
        public BillingResponse Dummy_ChargeUserForSubscription(double iPrice, string sCurrency, string sSubscriptionID, string sUserIP, string couponCode, string sExtraParameters)
        {
            ...
            try
            {
                ...
                response = Core.ConditionalAccess.Module.CC_DummyChargeUserForSubscription(groupId, UsersService.Instance.GetUserID(), iPrice, sCurrency, sSubscriptionID, couponCode, sUserIP, sExtraParameters, sLocaleCountry, sLocaleLanguage, sLocaleDevice);
        ...
        public string GetLicensedLinkWithCoupon(int iFileID, string sBaseURL, string sUserIP, string sCoupon)
        {
            ...
            try
            {
                ...
                sRet = Core.ConditionalAccess.Module.GetLicensedLinkWithCoupon(groupId, UsersService.Instance.GetUserID(), iFileID, sBaseURL, sUserIP, string.Empty, sLocaleCountry, sLocaleLanguage, sLocaleDevice, sCoupon);
            }
            catch (Exception ex)
            {
        ...
        public int GetCustomDataID(int type, string method, double dPrice, string sCurrencyCode, int campID, string couponCode, int assetID, string sModuleCode, string userIP, string overrideEndDate, string previewModuleID)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.GetCustomDataID(groupId, UsersService.Instance.GetUserID(), dPrice, sCurrencyCode, assetID, sModuleCode, campID.ToString(), couponCode, method, userIP, sLocaleCountry, sLocaleLanguage, sLocaleDevice, type, overrideEndDate, previewModuleID);
            }
            catch (Exception ex)
            {
        ...
        public PrePaidHistoryResponse GetUserPrePaidHistory(int numOfItems)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.GetUserPrePaidHistory(groupId, UsersService.Instance.GetUserID(), numOfItems);
            }
            catch (Exception ex)
            {
        ...
        public PrePaidResponse PP_ChargeUserForMediaFile(double price, string currencyCode, int fileId, int ppvModuleCode, string couponCode, string userIP)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.PP_ChargeUserForMediaFile(groupId, UsersService.Instance.GetUserID(), price, currencyCode, fileId, ppvModuleCode.ToString(), couponCode, userIP, string.Empty,
        ...
        public PrePaidResponse PP_ChargeUserForSubscription(double price, string currencyCode, int subCode, string couponCode, string userIP)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.PP_ChargeUserForSubscription(groupId, UsersService.Instance.GetUserID(), price, currencyCode, subCode.ToString(), couponCode, userIP, string.Empty,
                    string.Empty, string.Empty, string.Empty);
            }
            catch (Exception ex)
            {
        ...
        public PrePaidPricesContainer[] GetPrePaidPrices(string[] prepaids, string currencyCode, string couponCode)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.GetPrePaidPrices(groupId, prepaids, UsersService.Instance.GetUserID(), couponCode, string.Empty, string.Empty, string.Empty);
            }
            catch (Exception ex)
            {
        ...
        public UserPrePaidContainer GetUserPrePaidStatus(string currencyCode)
        {
            ...
            try
            {
                retVal = Core.ConditionalAccess.Module.GetUserPrePaidStatus(groupId, UsersService.Instance.GetUserID(), currencyCode);
            }
            catch (Exception ex)
            {
        ...
        public bool ActivateCampaign(long campID, string mediaLink, int mediaID, string voucherRecName, string voucherRecEmail, string voucherSendName, string voucherSendEmail)
        {
            ...
            try
            {
                return Core.ConditionalAccess.Module.ActivateCampaign(groupId, (int)campID, new CampaignActionInfo()
                {
                    ...
                    m_voucherReceipents = new List<VoucherReceipentInfo>() { new VoucherReceipentInfo() { m_emailAdd = voucherRecEmail, m_receipentName = voucherRecName } }
                });
            }
            catch (Exception ex)
            {
        ...
        public CampaignActionInfo ActivateCampaignWithInfo(long campID, string inviteHash)
        {
            ...
            try
            {
                return Core.ConditionalAccess.Module.ActivateCampaignWithInfo(groupId, (int)campID, new CampaignActionInfo()
                {
                    m_siteGuid = int.Parse(UsersService.Instance.GetUserID()),
                    m_socialInviteInfo = !string.IsNullOrEmpty(inviteHash) ? new SocialInviteInfo() { m_hashCode = inviteHash } : default(SocialInviteInfo)
                });
            }
            catch (Exception ex)
            {
        ...
        public string GetItemLeftViewLifeCycle(string m_wsUserCa, string m_wsPassCa, string m_mediaFileGuid, string m_userName, string m_country, string m_language, string m_device, bool bIsCoGuid)
        {
            ...
            try
            {
                return Core.ConditionalAccess.Module.GetItemLeftViewLifeCycle(groupId, m_mediaFileGuid, m_userName, bIsCoGuid, m_country, m_language, m_device);
            }
            catch (Exception ex)
            {
        ...
        public ChangeSubscriptionStatus ChangeSubscription(string sSiteGuid, int nOldSubscription, int nNewSubscription, string udid)
        {
            ...
            try
            {
                return Core.ConditionalAccess.Module.ChangeSubscription(groupId, sSiteGuid, nOldSubscription, nNewSubscription, udid);
            }
            catch (Exception ex)
            {
