...
namespace TVPPro.SiteManager.Helper
{
    public class ORCAHelper
    {
        ...
        public static ORCAGalleryResponse getVodORCAResponseByCategory(ORCA_CALLS ORCACallType, string categoryId)
        {
            ...
            try
            {
                response = WebRequestHelper.SendRequest<string>(string.Format("{0}GetRecommendationsByGallery", TVPAPI_BASE_URL), JsonConvert.SerializeObject(requestData), ORCA_REQUEST_TIMEOUT);
        ...
        private static ORCAGalleryResponse castResponse(string response)
        {
            ...
            try
            {
                ...
                {
                    object respoinseObj = JsonConvert.DeserializeObject(response);
