...
namespace Tvinci.Data.DataLoader
{
    ...
    public sealed class LoaderCacheLite : ILoaderCache
    {
        ...
        {
            int duration;
            if (int.TryParse(System.Configuration.ConfigurationManager.AppSettings["Tvinci.DataLoader.CacheLite.DurationInMinutes"], out duration))
            {
            ...
            bool useCache;
            if (bool.TryParse(System.Configuration.ConfigurationManager.AppSettings["Tvinci.DataLoader.CacheLite.ShouldUseCache"], out useCache))
            {
        ...
        public bool TryGetData<TData>(string uniqueKey, out TData data)
        {
            ...
            object result = CachingManager.CachingManager.GetCachedData(uniqueKey);
            if (result is TData)
            {
                data = (TData)result;
        ...
        public void AddData(string uniqueKey, object data, string[] categories, int cacheDuration)
        {
            if (ShouldUseCache)
            {
                ...
                CachingManager.CachingManager.SetCachedData(uniqueKey, data, cacheDuration * 60, System.Runtime.Caching.CacheItemPriority.Default, 0, true);
        ...
        private void cacheRemove(string key, object value)
        {
