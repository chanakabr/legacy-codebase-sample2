...
namespace TVPApiModule.Objects.Authorization
{
    public class KS
    {
        ...
        {
            int relativeExpiration = (int)TimeHelper.ConvertToUnixTimestamp(DateTime.UtcNow) + expiration;
        ...
        public static KS CreateKSFromEncoded(byte[] encryptedData, int groupId, string secret, string ksVal, KSVersion ksType, string secretFallback = null)
        {
            ...
            byte[] fieldsWithHashBytes = EncryptionUtils.AesDecrypt(secret, encryptedData.Skip(fieldsWithRandomIndex).ToArray(), BLOCK_SIZE);
            ...
            if (System.Text.Encoding.ASCII.GetString(hash) != System.Text.Encoding.ASCII.GetString(EncryptionUtils.HashSHA1(fieldsWithRandom)))
            {
                if (!string.IsNullOrEmpty(secretFallback))
                {
                    fieldsWithHashBytes = EncryptionUtils.AesDecrypt(secretFallback, encryptedData.Skip(fieldsWithRandomIndex).ToArray(), BLOCK_SIZE);
                ...
                else
                {
                    switch (pair[0])
                    {
                        ...
                        case "e":
                            ...
                            ks.expiration = TimeHelper.ConvertFromUnixTimestamp(expiration);
                            break;
                        case "u":
                            ks.userId = pair[1];
                            break;
        ...
        */
        ...
        public static KS CreateKSFromApiToken(APIToken token, string tokenVal)
        {
            KS ks = new KS()
            {
                ...
                userId = token.SiteGuid,
                sessionType = token.IsAdmin ? KalturaSessionType.ADMIN : KalturaSessionType.USER,
                expiration = TimeHelper.ConvertFromUnixTimestamp(token.AccessTokenExpiration),
                data = KSUtils.PrepareKSPayload(new KS.KSData() { UDID = token.UDID }),
