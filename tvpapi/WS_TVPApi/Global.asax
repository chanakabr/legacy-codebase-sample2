...
<script RunAt="server">
    ...
    {
        bool verifyHttps = Convert.ToBoolean(ConfigurationManager.AppSettings.Get("verifyHttps"));
        if (!verifyHttps)
        {
    ...
    {
        if (Sender is ASP.global_asax && (Sender as ASP.global_asax).Context.Items.Keys.Count == 0) return;
