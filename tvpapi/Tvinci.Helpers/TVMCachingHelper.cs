...
namespace Tvinci.Helpers
{
    public class TVMCachingHelper
    {
        ...
        private void sync()
        {
            ...
            try
            {
                DatabaseDirectAdapter<DataTable> q = new DatabaseDirectAdapter<DataTable>(delegate(TVPApi.ODBCWrapper.DataSetSelectQuery query)
                {
        ...
        private void UpdateTVMOnline(string status)
        {
            ...
            try
            {
                uQuery += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("TVMOnline", status);
                uQuery += TVPApi.ODBCWrapper.Parameter.NEW_PARAM("UPDATE_DATE", DateTime.Now);
