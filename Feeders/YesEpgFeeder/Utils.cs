...
namespace YesEpgFeeder
{
    public static class Utils
    {
        ...
        internal static void DeleteProgramsByChannelAndDate(int channelID, DateTime dDate, int ParentGroupID)
        {
            DateTime dProgEndDate = dDate.AddDays(1).AddMilliseconds(-1);
            ...
            DeleteScheduleProgramByDate(channelID, dDate);

            #endregion

            #region Delete all existing programs in ES that have start/end dates within the new schedule
            bool resDelete = DeleteEPGDocFromES(ParentGroupID, channelID, lDates);
            #endregion
        }
        ...
        private static bool DeleteEPGDocFromES(int parentGroupID, int channelID, List<DateTime> lDates)
        {
        ...
        internal static bool UpdateEpgIndex(List<ulong> ulProgram, int ParentGroupID, eAction eAction)
        {
            ...
            try
            {
                result = ImporterImpl.UpdateEpgIndex(ulProgram, ParentGroupID, eAction);
                return result;
            }
            catch (Exception ex)
            {
        ...
        internal static DateTime ParseEPGStrToDate(string date, string programtime)
        {
        ...
        private static Dictionary<string, List<string>> GetEpgProgramMetas(List<FieldTypeEntity> FieldEntityMapping)
        {
            ...
            var MetaFieldEntity = from item in FieldEntityMapping
                                  where item.FieldType == FieldTypes.Meta && item.XmlReffName.Capacity > 0 && item.Value != null && item.Value.Count > 0
                                  select item;
        ...
        private static Dictionary<string, List<string>> GetEpgProgramTags(List<FieldTypeEntity> FieldEntityMapping)
        {
            ...
            var TagFieldEntity = from item in FieldEntityMapping
                                 where item.FieldType == FieldTypes.Tag && item.XmlReffName.Capacity > 0 && item.Value != null && item.Value.Count > 0
                                 select item;
        ...
        internal static void GenerateTagsAndValues(EpgCB epg, List<FieldTypeEntity> FieldEntityMapping, ref Dictionary<int, List<string>> tagsAndValues)
        {
            foreach (string tagType in epg.Tags.Keys)
            {
                string tagTypel = tagType.ToLower();
                int tagTypeID = 0;
                List<FieldTypeEntity> tagField = FieldEntityMapping.Where(x => x.FieldType == FieldTypes.Tag && x.Name.ToLower() == tagTypel.ToLower()).ToList();
        ...
        internal static Dictionary<int, List<KeyValuePair<string, int>>> getTagTypeWithRelevantValues(int nGroupID, List<FieldTypeEntity> FieldEntityMappingTags, Dictionary<int, List<string>> tagsAndValues)
        {
        ...
        internal static void UpdateMetasPerEPG(ref DataTable dtEpgMetas, EpgCB epg, List<FieldTypeEntity> FieldEntityMappingMetas, int nUpdaterID)
        {
        ...
        internal static void UpdateExistingTagValuesPerEPG(EpgCB epg, List<FieldTypeEntity> FieldEntityMappingTags, ref DataTable dtEpgTags, ref DataTable dtEpgTagsValues, Dictionary<int, List<KeyValuePair<string, int>>> TagTypeIdWithValue, ref Dictionary<KeyValuePair<string, int>, List<string>> newTagValueEpgs, int nUpdaterID)
        {
        ...
        internal static void InsertNewTagValues(Dictionary<string, EpgCB> epgDic, DataTable dtEpgTagsValues, ref DataTable dtEpgTags,
