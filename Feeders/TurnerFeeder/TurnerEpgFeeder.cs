    ...
    public class TurnerTasker : ScheduledTasks.BaseTask
        {
            ...
            private string GetTurnerEpgString(string sChannelName)
            {
                ...
                Logger.Logger.Log("GetTurnerEpgString", string.Format("{0}", epgJsonRes), "TurnerEpgFeeder");
            ...
            {
                Logger.Logger.Log("Start task", string.Format("Group:{0}, FromDate:{1}, ToDate:{2}", m_groupID, m_fromDate, m_toDate), "TurnerEpgFeeder");
                ...
                try
                {
                    ...
                    {
                        Logger.Logger.Log("Ingest fail", "No EPG channels found", "TurnerEpgFeeder");
                        return false;
                    }
                    ...
                    foreach (KeyValuePair<int, string> chIdName in dEpgChannels)
                    {
                        ...
                        {
                            Logger.Logger.Log("Ingest warning", string.Format("No EPG data for channel {0}", chName), "TurnerEpgFeeder");
                            continue;
                        }
                ...
                {
                    Logger.Logger.Log("Ingest fail", ex.ToString(), "TurnerEpgFeeder");
                }
                ...
                Logger.Logger.Log("Ending task,", d.ToString("yyyy-MM-ddTHH:mm:ss"), "TurnerEpgFeeder");
                return true;
            }
            ...
            private bool SaveChannel(int channelID, List<TurnerEpgObj> programs)
            {
                try
                {
                    if (channelID > 0)
                    {
                        Logger.Logger.Log("Turner SaveChannels", string.Format("START EPG Channel = {0}", channelID), "TurnerEpgFeeder");
                        ...
                        for (int i = 0; i < programs.Count; i++)
                        {
                            ...
                            {
                                Logger.Logger.Log("Dates Error", string.Format("channelID={0}, turnerID={1}, startDate={2}, endDate={3}, TvinciChannelID={4}",
                                        channelID, programs[i].id, dProgStartDate, dProgEndDate, channelID), "TurnerEpgFeeder");
                            }
                        ...
                        Logger.Logger.Log("Turner", string.Format("END EPG Channel {0}", channelID), "TurnerEpgFeeder");
                    }
                    else
                    {
                        Logger.Logger.Log("Missing Channel ID", string.Format("GetExistChannel() ChannelID = {0}", channelID), "TurnerEpgFeeder");
                        return false;
                    }
                }
                catch (Exception exp)
                {
                    Logger.Logger.Log("Turner", string.Format("fail to insert channel to DB ex = {0}", exp.Message), "TurnerEpgFeeder");
                    return false;
                }
                finally
                {
            ...
            private void InsertEpgsDBBatches(ref Dictionary<string, EpgCB> epgDic, int groupID, int nCountPackage, List<FieldTypeEntity> FieldEntityMapping)
            {
                ...
                {
                    Logger.Logger.Log("InsertEpgsDBBatches", string.Format("Exception in inserting EPGs in group: {0}. exception: {1} ", groupID, exc.Message), "TurnerEpgFeeder");
                    return;
                }
            }
