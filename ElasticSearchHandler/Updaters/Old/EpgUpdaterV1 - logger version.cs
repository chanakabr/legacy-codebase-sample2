...
namespace ElasticSearchHandler.Updaters
{
    public class EpgUpdaterV1 : IElasticSearchUpdater
    {
        ...
        protected bool UpdateEpg(List<int> epgIds)
        {
            ...
            try
            {
                log.DebugFormat("UpdateEPG - start get group {0}", groupId);

                // get all languages per group
                Group group = GroupsCache.Instance().GetGroup(this.groupId);

                log.DebugFormat("UpdateEPG - finish get group {0}", groupId);
                ...
                log.Debug("UpdateEPG - start Get EPG Programs");
                ...
                for (int i = 0; i < epgIds.Count; i++)
                {
                    programsTasks[i] = Task.Factory.StartNew<List<EpgCB>>(
                        (epgId) =>
                        {
                            log.DebugFormat("UpdateEPG - start task of get epg program {0} before load cd", epgId);

                            cd.Load();

                            log.DebugFormat("UpdateEPG - start task of get epg program {0} after load cd", epgId);
