...
namespace RestAdaptersCommon
{
	public static class RestSharpHelper
	{
		...
		public static IRestResponse MakeRequest(this IRestClient client, IRestRequest request, int groupId)
		{
			return Task.Run(() => MakeRequestAsync(client, request, groupId)).ConfigureAwait(false).GetAwaiter().GetResult();
		}


		public static async Task<IRestResponse> MakeRequestAsync(this IRestClient client, IRestRequest request, int groupId)
		{
			...
			try
			{
				...
				{
					km.Event = "external";
					km.PartnerID = groupId.ToString();
					km.ClientTag = "REST-ADAPTER-REQUEST";
					km.Database = request.Resource;
					km.Action = client.BaseUrl.ToString();
					km.Table = "";
