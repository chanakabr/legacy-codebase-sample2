    ...
    stages {
        ...
        stage("Build"){
            steps{
                ...
                dir("tvmapps"){
                    bat (label:"Run MSBuild" , script:"\"${MSBUILD}\" \"Web Sites\\TVM\\TVM.csproj\" -m:4 -nr:False -t:Restore,Build,WebPublish"
        ...
        stage("Zip and Publish"){
            environment {
                    RELEASE_FULL_VERSION = sh(label:"Extract Full Verion Tag", script: 'cd tvmapps && ../Core/get-version-tag.sh', , returnStdout: true).trim()
