...
namespace Tvinci.Web.TVS.Controls.Menu
{
    ...
    public class MenuDataLoader : CustomAdapter<dsMenu.CategoryDataTable>
	{
		...
		void GetMenuCategory(ODBCWrapper.DataSetSelectQuery query)
		{
			...
			{
				query += ODBCWrapper.Parameter.NEW_PARAM("fc.type", "=", (int) Enum.Parse(typeof(eMenuType), m_MenuType.ToString()));
				query += " and ";
			}
			query += ODBCWrapper.Parameter.NEW_PARAM("fc.languageID", "=", ValueInDB);
			query += " order by fc.itemorder";
		}

		void GetCategoryItems(ODBCWrapper.DataSetSelectQuery query)
		{
			if (TableName == eMenuDBTable.Menu)
